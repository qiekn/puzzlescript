  ...
  (async () => {
    ...
    glob("./bin/Documentation/images/*.gif", {}, async function (er, files) {
      ...
      var css = fs.readFileSync(
        "./bin/Documentation/css/bootstrap.css",
        (encoding = "utf8"),
      );
      var min = cssmin(css);
      fs.writeFileSync(
      ...
      for (var i = 0; i < files.length; i++) {
        var fpath = files[i];
        corpus["source/" + fpath.slice(9)] = fs.readFileSync(
          fpath,
          (encoding = "utf-8"),
        );
      }
      var result = await minify(corpus, {
        sourceMap: {
      ...
      editor = editor.replace(
        /<script src="js\/[A-Za-z0-9_\/-]*\.js"><\/script>/g,
        "",
      );
      editor = editor.replace(
        /<!--___SCRIPTINSERT___-->/g,
        '<script src="js\/scripts_compiled.js"><\/script>',
      );
      editor = editor.replace(
        /<link rel="stylesheet" href="[A-Za-z0-9_\/-]*\.css">/g,
        "",
      );
      editor = editor.replace(
        /<!--CSSREPLACE-->/g,
      ...
      editor = editor.replace(
        /<!--BUILDNUMBER-->/g,
        `build ${buildnum.toString()}, ${d.getDate()}-${monthname[d.getMonth()]}-${d.getFullYear()}`,
      );
      ...
      //then bin one:
      standalone_raw = standalone_raw.replace(
        /<script src="js\/[A-Za-z0-9_\/-]*\.js"><\/script>/g,
        "",
      );
      standalone_raw = standalone_raw.replace(
