...
function adjustLevel(level, widthdelta, heightdelta) {
  ...
  for (let i = 0; i < level.n_tiles; ++i) level.setCell(i, bgMask);
...
function matchGlyph(inputmask, glyphAndMask) {
  ...
  logWarningNoLine(
    "Wasn't able to approximate glyph values for some tiles, using '.' as a placeholder.",
    false,
  );
  compiling = false;
  return ".";
...
function pollGamepads() {
  ...
  for (var i = 0; i < gamepads.length; i++) {
    ...
    if (buttonCheck(gamepad.buttons, 7) || buttonCheck(gamepad.buttons, 8)) {
      // menu button
      keyPressed(27); // exit
    }
    ...
    if (
      ...
      buttonCheck(gamepad.buttons, 12)
    ) {
      // D-pad up
    ...
    if (
      ...
      axisCheck(gamepad.axes, 7, 1) ||
      buttonCheck(gamepad.buttons, 13)
    ) {
      // D-pad down
    ...
    if (
      ...
      buttonCheck(gamepad.buttons, 14)
    ) {
      // D-pad left
    ...
    if (
      ...
      axisCheck(gamepad.axes, 6, 1) ||
      buttonCheck(gamepad.buttons, 15)
    ) {
      // D-pad right
...
function update() {
  ...
  if (keybuffer.length > 0) {
    ...
    const ticklength = throttle_movement
      ? repeatinterval
