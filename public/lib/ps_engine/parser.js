...
function TooManyErrors() {
  const message = compiling
    ? "Too many errors/warnings; aborting compilation."
    : "Too many errors/warnings; noping out.";
...
let codeMirrorFn = function () {
  ...
  function checkNameDefined(state, candname) {
    ...
    logError(
      `You're talking about ${candname.toUpperCase()} but it's not defined anywhere.`,
      state.lineNumber,
    );
  }
  ...
  function processLegendLine(state, mixedCase) {
    ...
    if (splits.length === 1) {
      ...
      logError(
        `Incorrect format of legend - should be one of "A = B", "A = B or C [ or D ...]", "A = B and C [ and D ...]", but it looks like you have a dangling "${state.current_line_wip_array[state.current_line_wip_array.length - 1].toUpperCase()}"?`,
        state.lineNumber,
      );
      ok = false;
    } else {
      ...
      if (alreadyDefined !== null) {
        logError(
          `Name "${candname.toUpperCase()}" already in use (on line <a onclick="jumpToLine(${alreadyDefined.lineNumber});" href="javascript:void(0);"><span class="errorTextLineNumber">line ${alreadyDefined.lineNumber}</span></a>).`,
          state.lineNumber,
        );
        ok = false;
      }
      ...
      for (let i = 2; i < splits.length; i += 2) {
        ...
        for (let j = 2; j < i; j += 2) {
          ...
          if (oname === nname) {
            logWarning(
              "You're repeating the object " +
                oname.toUpperCase() +
                " here multiple times on the RHS.  This makes no sense.  Don't do that.",
              state.lineNumber,
            );
          }
        }
      ...
      if (splits.length === 3) {
        ...
        let substitutor = function (n) {
          ...
          for (let i = 0; i < state.legend_aggregates.length; i++) {
            ...
            if (a[0] === n) {
              logError(
                `Cannot define a property (something defined in terms of 'or') in terms of an aggregate (something that uses 'and').  In this case, you can't define "${splits[0]}" in terms of "${n}".`,
                state.lineNumber,
              );
              malformed = true;
              return [];
  ...
  function processSoundsLine(state) {
    ...
    if (
      state.current_line_wip_array[state.current_line_wip_array.length - 1] ===
      "ERROR"
    ) {
    } else {
  ...
  let fullSpriteMatrix = ["00000", "00000", "00000", "00000", "00000"];
  ...
  return {
    copyState: function (state) {
      ...
      let original_case_namesCopy = Object.assign(
        {},
        state.original_case_names,
      );
      let original_line_numbersCopy = Object.assign(
        {},
        state.original_line_numbers,
      );
        ...
        if (sol && sectionNameMatches) {
          ...
          if (sectionIndex == 0) {
            ...
            if (sectionIndex === -1) {
              ...
              logError(
                'no such section as "' + state.section.toUpperCase() + '".',
                state.lineNumber,
              );
            } else {
              logError(
                'section "' +
                  ...
                  sectionNames[sectionIndex - 1].toUpperCase() +
      ...
      switch (state.section) {
        case "objects": {
          let tryParseName = function () {
            ...
            if (match_name == null) {
              ...
              for (let i = 0; i < state.legend_synonyms.length; i++) {
                ...
                if (entry[0] == candname) {
                  logError(
                    'Name "' + candname.toUpperCase() + '" already in use.',
                    state.lineNumber,
                  );
                }
              }
          ...
          switch (state.objects_section) {
              ...
              if (match_color == null) {
                ...
                logError(
                  "Was looking for color for object " +
                    state.objects_candname.toUpperCase() +
                    ', got "' +
                    str +
                    '" instead.',
                  state.lineNumber,
            ...
            case 3: {
              ...
              if (
                spritematrix.length === 5 &&
                spritematrix[spritematrix.length - 1].length === 5
              ) {
                state.objects_section = 0;
              }
          ...
          if (state.tokenIndex === 0) {
            ...
            if (match_name === null) {
              ...
              if (candname === "and" || candname === "or") {
                ...
                logError(
                  `Expected and 'AND' or an 'OR' here, but got ${candname.toUpperCase()} instead. In the legend, define new items using the equals symbol - declarations must look like 'A = B' or 'A = B and C' or 'A = B or C'.`,
                  state.lineNumber,
                );
                resultToken = "ERROR";
        ...
        case "sounds": {
          ...
          if (
            ...
            state.current_line_wip_array[
              state.current_line_wip_array.length - 1
            ] === "ERROR"
          ) {
            // match=stream.match(reg_notcommentstart, true);
            ...
            if (match == null) {
              ...
              if (match == null) {
                ...
                if (!wordAlreadyDeclared(state, matched_name)) {
                  ...
                  state.current_line_wip_array.push("ERROR");
                  logError(
                    `unexpected sound token "${matched_name}".`,
                    state.lineNumber,
                  );
                } else {
                  tokentype = "NAME";
            ...
            let is_soundevent =
              state.current_line_wip_array[0][1] === "SOUNDEVENT";
            ...
            if (is_soundevent) {
              ...
              logError(
                `I wasn't expecting anything after the sound declaration ${state.current_line_wip_array[state.current_line_wip_array.length - 1][0].toUpperCase()} on this line, so I don't know what to do with "${match[0].trim().toUpperCase()}" here.`,
                state.lineNumber,
              );
              tokentype = "ERROR";
              state.current_line_wip_array.push("ERROR");
              ...
              if (is_seedonright) {
                ...
                let directional_verb =
                  soundverbs_directional.indexOf(
                    state.current_line_wip_array[1][0],
                  ) >= 0;
                if (directional_verb) {
                  //match seed or direction
                  let is_direction = stream.match(
                    reg_sounddirectionindicators,
                    true,
                  );
                  if (is_direction !== null) {
                    ...
                    if (is_seed !== null) {
                      ...
                      logError(
                        `Ah I was expecting direction or a sound seed here after ${state.current_line_wip_array[state.current_line_wip_array.length - 1][0].toUpperCase()}, but I don't know what to make of "${match[0].trim().toUpperCase()}".`,
                        state.lineNumber,
                      );
                      tokentype = "ERROR";
                      state.current_line_wip_array.push("ERROR");
                  ...
                  if (is_seed !== null) {
                    ...
                    //depending on whether the verb is directional or not, we log different errors
                    logError(
                      `Ah I was expecting a sound seed here after ${state.current_line_wip_array[state.current_line_wip_array.length - 1][0].toUpperCase()}, but I don't know what to make of "${match[0].trim().toUpperCase()}".`,
                      state.lineNumber,
                    );
                    tokentype = "ERROR";
                    state.current_line_wip_array.push("ERROR");
          ...
          if (match_name === null) {
            ...
            if (candname === "background") {
              if (
                ...
                state.collisionLayers[state.collisionLayers.length - 1].length >
                  0
              ) {
                logError(
            ...
            if (state.current_line_wip_array.indexOf(candname) >= 0) {
              let warningStr =
                'Object "' +
                candname.toUpperCase() +
                "\" included explicitly multiple times in the same layer. Don't do that innit.";
        ...
        case "winconditions": {
          if (sol) {
            ...
            let filtered = splitted.filter(function (v) {
              return v !== "";
            });
          ...
          if (match === null) {
            ...
            if (state.tokenIndex === 0) {
              if (reg_winconditionquantifiers.exec(candword)) {
                ...
                logError(
                  'Expecting the start of a win condition ("ALL","SOME","NO") but got "' +
              ...
              logError(
                "Error in win condition: I don't know what to do with " +
                  candword.toUpperCase() +
                  ".",
                state.lineNumber,
              );
        ...
        case "levels": {
          if (sol) {
            ...
            } else if (stream.match(/[\p{Z}\s]*message[\p{Z}\s]*/u, true)) {
        ...
        default: {
          ...
          if (state.tokenIndex == 0) {
            ...
            if (match !== null) {
                ...
                if (
                  ...
                  if (m2 !== null) {
                    ...
                    if (token in state.metadata_lines) {
                      ...
                      logWarning(
                        `You've already defined a ${token.toUpperCase()} in the prelude on line <a onclick="jumpToLine(${otherline})>${otherline}</a>.`,
                        state.lineNumber,
                      );
                    }
                  ...
                  if (m2 !== null) {
                    ...
                    logWarning(
                      "MetaData " +
                        token.toUpperCase() +
                        " doesn't take any parameters, but you went and gave it \"" +
                        extra +
                        '".',
                      state.lineNumber,
                    );
              ...
              //garbage found
              logError(
                `Unrecognised stuff "${stream.string}" in the prelude.`,
                state.lineNumber,
              );
            }
          } else {
