...
function isColor(str) {
  ...
  if (REGEX_HEX.test(str)) return true;
...
function generateExtraMembers(state) {
  ...
  for (let k_i = 0; k_i < state_object_keys_l; k_i++) {
    ...
    if (o.spritematrix.length === 0) {
      ...
      if (
        o.spritematrix.length !== 5 ||
        o.spritematrix[0].length !== 5 ||
        o.spritematrix[1].length !== 5 ||
        o.spritematrix[2].length !== 5 ||
        o.spritematrix[3].length !== 5 ||
        o.spritematrix[4].length !== 5
      ) {
        logWarning(
...
function processRuleString(rule, state, curRules) {
  ...
  let tokens = line.split(/\s/).filter(function (v) {
    return v !== "";
  });
        ...
        if (token === "[") {
          ...
          if (!incellrow) {
            ...
            for (let j = 0; j < curcell.length; j += 2) {
              if (curcell[j + 1] === token) {
                logError(
                  `You cannot specify the same object more than once in a single cell (in this case ${token} occurs mutliple times).`,
                  lineNumber,
                );
          ...
          if (token === "message") {
            ...
            if (commandwords_sfx.indexOf(token) >= 0) {
              ...
              if (!found) {
                logWarning(
                  'Sound effect "' + token + '" not defined.',
                  lineNumber,
                );
              }
            }
...
function deepCloneHS(HS) {
  let cloneHS = HS.map(function (arr) {
        ...
        for (let l = 0; l < cell.length; l += 2) {
          ...
          if (item.startsWith("no")) {
            ...
            if (no_name_mask.anyBitsInCommon(objects_present_mask)) {
              logError(
                `You have specified that there should be NO ${no_name.toUpperCase()} but there is also a requirement that ${objects_present.join(", ").toUpperCase()} be here.  This is a mistake right?`,
                rule.lineNumber,
              );
            }
          }
      ...
      for (let k = 0; k < lhs_group_len; k++) {
        ...
        for (let l = 0; l < cell.length; l += 2) {
          ...
          if (item.startsWith("no")) {
            ...
            let no_object_layer_mask = new BitVec(
              Math.ceil(LAYER_COUNT / 32) | 0,
            );
            ...
            const object_layers_disjoint =
              !no_object_mask.anyBitsInCommon(required_objects);
            const no_object_covered_by_required_layers =
              no_object_layer_mask.bitsSetInArray(required_layers.data);

            if (
              no_object_covered_by_required_layers &&
              object_layers_disjoint
            ) {
              const obs_present = [];
              ...
              logWarning(
                `You have specified that there should be NO ${no_name.toUpperCase()} but there is also a requirement that ${obs_present.join(", ").toUpperCase()} be there, which collectively occupies the same layers (Layers ${layers_present.map((l) => l + 1).join(", ")}), so you can leave this out.`,
                rule.lineNumber,
              );
              cell.splice(l, 2);
              l -= 2;
          ...
          for (let prop_n = 0; prop_n < properties.length; ++prop_n) {
            ...
            for (let l = 0; l < aliases.length; l++) {
              ...
              if (cur_rule.propertyReplacement) {
                const cur_rule_propertyReplacement_keys = Object.keys(
                  cur_rule.propertyReplacement,
                );
                const cur_rule_propertyReplacement_keys_l =
                  cur_rule_propertyReplacement_keys.length;
                for (
                  let k_i = 0;
                  k_i < cur_rule_propertyReplacement_keys_l;
                  k_i++
                ) {
...
function concretizeMovingRule(state, rule, lineNumber) {
          ...
          if (movings.length > 0) {
            ...
            for (let l = 0; l < concreteDirs.length; l++) {
              ...
              if (cur_rule.movingReplacement) {
                const cur_rule_movingReplacement_keys = Object.keys(
                  cur_rule.movingReplacement,
                );
                const cur_rule_movingReplacement_keys_l =
                  cur_rule_movingReplacement_keys.length;
                for (
                  let k_i = 0;
                  k_i < cur_rule_movingReplacement_keys_l;
                  k_i++
                ) {
              ...
              if (cur_rule.aggregateDirReplacement) {
                const cur_rule_aggregateDirReplacement_keys = Object.keys(
                  cur_rule.aggregateDirReplacement,
                );
                const cur_rule_aggregateDirReplacement_keys_l =
                  cur_rule_aggregateDirReplacement_keys.length;
                for (
                  let k_i = 0;
                  k_i < cur_rule_aggregateDirReplacement_keys_l;
                  k_i++
                ) {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    const ambiguous_movement_names_dict_keys = Object.keys(
      ambiguous_movement_names_dict,
    );
    const ambiguous_movement_names_dict_keys_l =
      ambiguous_movement_names_dict_keys.length;
    ...
    for (let rowIndex = 0; rowIndex < rule.lhs.length; rowIndex++) {
      ...
      for (let colIndex = 0; colIndex < cellrow_l.length; colIndex++) {
        ...
        for (let i = 0; i < cell_l.length; i += 2) {
          ...
          const layerIndex = object
            ? object.layer | 0
            : state.propertiesSingleLayer[object_name];

          if (typeof layerIndex === "undefined") {
            logError(
              `Oops! ${object_name.toUpperCase()} not assigned to a layer.`,
              rule.lineNumber,
            );
          }
          ...
          if (object_dir === "no") {
            ...
            movementMask.ishiftor(
              object_dir === "stationary" ? 0x1f : dirMasks[object_dir],
              STRIDE_5 * layerIndex,
            );
          }
        }
        ...
        if (!bitVectors.objectsPresent.iszero()) {
          if (
            bitVectors.objectsPresent.anyBitsInCommon(bitVectors.objectsMissing)
          ) {
            const ln = rule.lineNumber;

            const overlap = getOverlapObjectNames(
              state,
              bitVectors.objectsPresent,
              bitVectors.objectsMissing,
            );
            var x_no_x_list = overlap
              .map((x) => x + " NO " + x)
              .join(", ")
              .toUpperCase();
            logWarning(
              `This rule has something amounting to "${x_no_x_list}" on the left-hand-side,which can never match, so the rule is getting removed during compilation.`,
              rule.lineNumber,
            );
        ...
        for (let i = 0; i < cell_r.length; i += 2) {
          ...
          if (object_dir === "random") {
            if (object_name in state.objectMasks) {
              ...
              const values = state.propertiesDict.hasOwnProperty(object_name)
                ? state.propertiesDict[object_name]
                : [object_name];
              ...
              for (const subobject of values) {
                ...
                if (existingname !== null) {
                  const [o1, o2] = [
                    subobject.toUpperCase(),
                    existingname.toUpperCase(),
                  ];
                  if (o1 !== o2) {
                    logWarning(
                      `This rule may try to spawn a ${o1} with random, but also requires a ${o2} be here, which is on the same layer - they shouldn't be able to coexist!`,
                      rule.lineNumber,
                    );
                  }
                }
              ...
              logError(
                `You want to spawn a random "${object_name.toUpperCase()}", but I don't know how to do that`,
                rule.lineNumber,
              );
            }
            continue;
          ...
          if (object_dir === "no") {
            ...
            const existingname =
              layersUsed_r[layerIndex] || layersUsedRand_r[layerIndex];
            if (existingname !== null && !rule.hasOwnProperty("discard")) {
              logError(
                `Rule matches object types that can't overlap: "${object_name.toUpperCase()}" and "${existingname.toUpperCase()}".`,
                rule.lineNumber,
              );
            }
            ...
            if (object_dir.length > 0) {
              rhsBitVectors.postMovementsLayerMask_r.ishiftor(
                0x1f,
                STRIDE_5 * layerIndex,
              );
            }
            ...
            if (object) {
              ...
              rhsBitVectors.objectlayers_r.ishiftor(
                0x1f,
                STRIDE_5 * layerIndex,
              );
            }
            ...
            if (object_dir === "stationary") {
              rhsBitVectors.movementsClear.ishiftor(
                0x1f,
                STRIDE_5 * layerIndex,
              );
            } else if (object_dir === "randomdir") {
              rhsBitVectors.randomDirMask_r.ishiftor(
                dirMasks[object_dir],
                STRIDE_5 * layerIndex,
              );
            } else {
              rhsBitVectors.movementsSet.ishiftor(
                dirMasks[object_dir],
                STRIDE_5 * layerIndex,
              );
            }
        ...
        if (
          !bitVectors.objectsPresent.bitsSetInArray(
            rhsBitVectors.objectsSet.data,
          )
        ) {
          rhsBitVectors.objectsClear.ior(bitVectors.objectsPresent);
        }
        if (
          !bitVectors.movementsPresent.bitsSetInArray(
            rhsBitVectors.movementsSet.data,
          )
        ) {
        ...
        for (let layerIndex = 0; layerIndex < layerCount; layerIndex++) {
          if (
            layersUsed_l[layerIndex] !== null &&
            layersUsed_r[layerIndex] === null
          ) {
        ...
        const hasChanges =
          !rhsBitVectors.objectsClear.iszero() ||
...
function getMaskFromName(state, name) {
  ...
  if (objectMask.iszero()) {
    logErrorNoLine(
      `Error, didn't find any object called ${name}, either in the objects section, or the legends section.`,
    );
  }
...
function generateMasks(state) {
  ...
  let synonyms_and_properties = state.legend_synonyms.concat(
  ...
  const getCoords = function (val, lineNumber) {
    ...
    if (coords.length !== 2) {
      ...
      if (
        ...
        logWarning(
          `Couldn't understand the dimensions given to me (you gave "${val}") - should be of the form AxB.`,
          lineNumber,
        );
        return null;
      } else {
        if (intcoords[0] <= 0 || intcoords[1] <= 0) {
          logWarning(
            `The dimensions given to me (you gave "${val}") are baad - they should be > 0.`,
            lineNumber,
          );
        }
        return intcoords;
...
function processWinConditions(state) {
  ...
  for (let i = 0; i < state.winconditions.length; i++) {
    ...
    if (wincondition.length <= 2) {
      logError(
        'Win conditions is badly formatted - needs to look something like "No Fruit", "All Target On Crate", "Some Fruit", "Some Gold on Chest", "No Gold on Chest", or the like.',
        lineNumber,
      );
  ...
  for (let i = 0; i < state.rules.length; i++) {
    ...
    if (outsideLoop) {
      ...
      if (loopIndex + 1 < state.loops.length) {
        ...
        while (
          loopIndex + 1 < state.loops.length &&
          loopEnd[0] < rule.lineNumber
        ) {
          loopIndex += 2;
  ...
  for (let j = 0; j < state.loops.length; j += 2) {
    ...
    for (let group_i = 0; group_i < rulegroup_collection.length; group_i++) {
      ...
      if (firstRuleLine <= loop_start_line && loop_start_line <= lastRuleLine) {
        ...
        firstRuleLine <= loop_end_line &&
        loop_end_line <= lastRuleLine
      ) {
        logError(
          "Found a loop point in the middle of a rule. You probably don't want to do this, right?",
          loop_start_line,
      ...
      let rule_in_loop =
        loop_start_line <= firstRuleLine && firstRuleLine <= loop_end_line;
      let rule_after_loop = loop_end_line < firstRuleLine;

      if (rule_after_loop) break;
      ...
      if (rule_in_loop) {
        ...
        if (
          ...
          loopPoint[group_i - 1] === init_group_index
        ) {
          loopPoint[group_i - 1] = undefined;
        }
...
function generateLoopPoints(state) {
  ...
  if (state.loops.length % 2 !== 0) {
    logError(
      ...
      state.loops[state.loops.length - 1][0],
    );
...
function generateSoundData(state) {
  ...
  for (let i = 0; i < state.sounds.length; i++) {
    ...
    if (t0 === "SOUNDEVENT") {
      ...
      let verb = v1;
      let directions = [];
      for (let j = 2; j < sound.length - 2; j++) {
        ...
        if (sound[j][1] === "DIRECTION") {
          ...
          logError(
            `Expected a direction here, but found instead "$(sound[j][0])".`,
            lineNumber,
          );
        }
      }
...
function formatHomePage(state) {
  ...
  if (isColor(state.fgcolor) === false) {
    logError(
      ...
      state.metadata_lines.text_color,
    );
    state.fgcolor = "#FFFFFF";
  }
  if (isColor(state.bgcolor) === false) {
    logError(
      ...
      state.metadata_lines.background_color,
    );
    state.bgcolor = "#000000";
  }
...
function addSpecializedFunctions(state) {
  ...
  state.moveEntitiesAtIndex = generate_moveEntitiesAtIndex(
    OBJECT_SIZE,
    MOVEMENT_SIZE,
  );
  state.calculateRowColMasks = generate_calculateRowColMasks(
    OBJECT_SIZE,
    MOVEMENT_SIZE,
  );
  state.resolveMovements = generate_resolveMovements(
    OBJECT_SIZE,
    MOVEMENT_SIZE,
    state,
  );
  state.matchCellRow = generateMatchCellRow(OBJECT_SIZE, MOVEMENT_SIZE);
  state.matchCellRowWildCard = generateMatchCellRowWildCard(
    OBJECT_SIZE,
    MOVEMENT_SIZE,
  );
  state.repositionEntitiesAtCell = generate_repositionEntitiesAtCell(
    OBJECT_SIZE,
    MOVEMENT_SIZE,
  );
}
