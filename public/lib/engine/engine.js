...
function getLevelSelectScreen(inserts) {
  const lines = ["[ ESC: Back ]                ", "Level Select", ""];
  const options = [];
  inserts.forEach((ins) => {
...
function hasStartedTheGame() {
  return (
    (curLevelNo > 0 ||
      curlevelTarget !== null ||
      storage_has(document.URL + "_checkpoint")) &&
    curLevelNo in state.levels
  );
}
...
function isContinueOptionSelected() {
  return (
    state.metadata.skip_title_screen ||
    (!state.metadata.continue_is_level_select &&
...
function isNewGameOptionSelected() {
  return (
    titleSelection == MENUITEM_NEWGAME || titleSelection == MENUITEM_STARTGAME
  );
}
...
function generateTitleScreen(hoverLine, scrollIncrement, selectLine) {
  ...
  if (titleMode === 0) {
    ...
    titleImage = selectLine
      ? fillAndHighlight(screen, -1, -1, screen.options[0])
      : fillAndHighlight(screen, screen.options[0]);
    titleSelection = selectLine ? MENUITEM_STARTGAME : null;
  } else {
    const playedGameBefore =
      hasStartedTheGame() || hasSolvedAtLeastOneSection();
    const options = [];
    options.push(
      playedGameBefore && !hasFinishedTheGame()
        ? MENUITEM_CONTINUE
        : MENUITEM_NEWGAME,
    );
    if (
      state.metadata.level_select &&
      (!state.metadata.continue_is_level_select || !playedGameBefore)
    )
    ...
    if (levelSelectScrollPos == 0) levelSelectScrollPos = screen.options[0];
    else if (
      scrollIncrement &&
      screen.options.includes(levelSelectScrollPos + scrollIncrement)
    )
      levelSelectScrollPos += scrollIncrement;

    titleImage = fillAndHighlight(
      screen,
    ...
    titleSelection = screen.options.includes(select)
      ? options[screen.options.indexOf(select)]
      : false; // todo: ???
  }
  if (debugSwitch.includes("menu"))
    console.log(
      `generateTitleScreen2`,
      `titleSelection=`,
      titleSelection,
      `levelSelectScrollPos=`,
      levelSelectScrollPos,
  ...
  const setImage = (n, text) => {
    ...
    if (state.keyhint_color) lineColorOverride[n] = state.keyhint_color;
  };
  if (state.metadata.text_controls) {
    ...
    const tclick =
      state.metadata.mouse_drag || state.metadata.mouse_rdrag
        ? " Click, Tap, or Drag to interact"
        : " Click or Tap to interact";
    setImage(
      10,
      IsMouseGameInputEnabled() ? tclick : " Arrow keys or WASD to move",
    );
    setImage(
      11,
      (state.metadata.noaction ? " X to select" : " X to action") +
        (state.metadata.norestart ? "" : ", R to restart"),
    );
    const tundo = IsMouseGameInputEnabled()
      ? " Z or Middle Mouse Button to undo"
      : " Z to undo";
  ...
  if (titleSplit.length > maxl) {
    ...
    if (!hoverLine)
      logWarning(
        `Game title is too long to fit on screen, truncating to ${maxl} lines.`,
        state.metadata_lines.title,
        true,
      );
  }
  titleSplit.forEach((line, x) => {
    titleImage[1 + x] = centerText(line.trim(), TITLE_WIDTH);
    if (state.title_color) lineColorOverride[1 + x] = state.title_color;
  });
  ...
  if (state.metadata.author) {
    ...
    if (split.length > 2) {
      ...
      if (!hoverLine)
        logWarning(
          "Author list too long to fit on screen, truncating to 2 lines.",
          state.metadata_lines.author,
          true,
        );
    }
    split.forEach((line, x) => {
      titleImage[3 + x] = line.trim().padStart(TITLE_WIDTH);
      if (state.author_color) lineColorOverride[3 + x] = state.author_color;
    });
  }
  redraw();
}
...
function generatePauseScreen(hoverLine, scrollIncrement, selectLine) {
  ...
  pauseSelection =
    (hoverLine >= 0 ? hoverLine : selectLine >= 0 ? selectLine : 0) -
    screen.options[0];
  redraw();
}
...
function fillAndHighlight(image, highlight, hover, select) {
  const ll = image.lines.map((l, x) =>
    x == select && image.options.includes(x)
      ? centerText(`# ${l} #`, TITLE_WIDTH, "#")
      : x == hover && image.options.includes(x)
        ? centerText(`> ${l} <`, TITLE_WIDTH)
        : x == highlight && image.options.includes(x)
          ? centerText(`# ${l} #`, TITLE_WIDTH)
          : centerText(l, TITLE_WIDTH),
  );
  ...
  if (titleSelection == null) {
    for (var i = 0; i < state.sections.length; i++) {
      if (state.sections[i].firstLevel > curLevelNo) {
        ...
        if (debugSwitch.includes("menu"))
          console.log(
            `gotoLevelSelect curLevelNo=${curLevelNo} titleSelection=${titleSelection}`,
          );
        break;
      }
    }
...
function generateLevelSelectScreen(hoverLine, scrollIncrement, selectLine) {
  ...
  if (state.metadata.level_select_lock) {
    ...
    if (state.metadata.level_select_unlocked_ahead !== undefined) {
      ...
      unlockedUntil =
        ...
        state.metadata.level_select_unlocked_rollover -
        1;
    } else {
      unlockedUntil += 1;
    }
  ...
  else if (levelHighlightLine > 3 && scrollIncrement < 0) levelHighlightLine--;
  else if (
    levelHighlightLine < 3 + amountOfLevelsOnScreen - 1 &&
    scrollIncrement > 0
  )
    levelHighlightLine++;
  else if (
    levelSelectScrollPos > 0 &&
    (levelHighlightLine == 3 || scrollIncrement < 0)
  )
    levelSelectScrollPos--;
  else if (
    levelSelectScrollPos + amountOfLevelsOnScreen < state.sections.length &&
    (levelHighlightLine == 11 || scrollIncrement > 0) &&
    !titleSelected
  )
    levelSelectScrollPos++;
  ...
  console.log(
    `titleSelected=${titleSelected} titleSelection=${titleSelection}`,
  );
  ...
  const showLines = lines.slice(
    levelSelectScrollPos,
    levelSelectScrollPos + amountOfLevelsOnScreen,
  );
  const screen = getLevelSelectScreen(showLines);
  if (debugSwitch.includes("menu"))
    console.log(
      `generateLevelSelectScreen2 titleSelection=${titleSelection}`,
      `levelSelectScrollPos=${levelSelectScrollPos}`,
      screen,
    );
  titleImage = fillAndHighlight(
    screen,
  ...
  titleImage[0] = (hoverLine == 0 ? "[  ESC:Back  ]" : " [ ESC:Back ] ").padEnd(
    TITLE_WIDTH,
  );
  if (levelSelectScrollPos > 0)
    titleImage[2] = (hoverLine == 2 ? "[  PREV  ]" : "[ PREV ] ").padStart(
      TITLE_WIDTH,
    );
  if (levelSelectScrollPos + amountOfLevelsOnScreen < lines.length)
    titleImage[12] = (hoverLine == 12 ? "[  NEXT  ]" : "[ NEXT ] ").padStart(
      TITLE_WIDTH,
    );
  redraw();
}
...
function gotoLink() {
  if (debugSwitch.includes("load"))
    console.log("gotoLink()", `stack:`, linkStack);
  if (solving) return;
...
function returnLink() {
  if (debugSwitch.includes("load"))
    console.log("returnLink()", `stack:`, linkStack);
  ...
  if (verbose_logging)
    consolePrint(
      `Returning to level ${linkEntry.backup.levelNo} (${htmlJump(level.lineNumber)}).`,
      true,
      level.lineNumber,
    );
...
function drawMessageScreen(message) {
  ...
  const screen = getMessageScreen(
    quittingMessageScreen
      ? ""
      : state.metadata.text_message_continue
        ? state.metadata.text_message_continue
        : IsMouseGameInputEnabled()
          ? "Click or X to continue"
          : "X to continue",
  );
  ...
  const lines = splitMessage.map((m) => {
    return state.metadata.message_text_align == "left"
      ? m.padEnd(TITLE_WIDTH)
      : state.metadata.message_text_align == "right"
        ? m.padStart(TITLE_WIDTH)
...
function loadLevelFromLevelDat(state, leveldat, randomseed, clearinputhistory) {
  if (debugSwitch.includes("load"))
    console.log(`loadLevelFromLevelDat()`, leveldat);
  if (randomseed == null) {
  ...
  if (leveldat.message) {
    ...
    if (verbose_logging)
      consolePrint(
        `Showing message (${htmlJump(leveldat.lineNumber)})`,
        true,
        leveldat.lineNumber,
      );
    ignoreNotJustPressedAction = true;
    ...
    if (verbose_logging)
      consolePrint(
        `GOTO (${htmlJump(leveldat.lineNumber)})`,
        true,
        leveldat.lineNumber,
      );
    ...
    if (verbose_logging)
      consolePrint(
        `Loading "${leveldat.section || leveldat.title}" (${htmlJump(leveldat.lineNumber)}).`,
        true,
        leveldat.lineNumber,
      ); //todo:
    RebuildLevelArrays();
    if (state !== undefined) {
      if (state.metadata.flickscreen !== undefined) {
        ...
        oldflickscreendat = [
          ...
          0,
          Math.min(
            state.metadata.smoothscreen.screenSize.width,
            curLevel.width,
          ),
          Math.min(
            state.metadata.smoothscreen.screenSize.height,
            curLevel.height,
          ),
        ];
      }
    ...
    if (!state.metadata.allow_undo_level) backups = [];
...
function loadLevelFromStateTarget(state, levelindex, target, randomseed) {
  if (debugSwitch.includes("load"))
    console.log(`loadLevelFromStateTarget(${levelindex},${target})`);
  var leveldat = target;
  if (verbose_logging)
    consolePrint(
      `Returning to checkpoint in "${state.levels[levelindex].section || state.levels[levelindex].title}".`,
    );
...
function loadLevelFromState(state, levelindex, randomseed) {
  if (debugSwitch.includes("load"))
    console.log(`loadLevelFromState(levelindex=${levelindex})`);
...
function tryLoadImages() {
  if (!state.metadata.load_images) return;
  customImages = {};
  ...
  state.metadata.load_images.split(" ").forEach((arg) => {
    ...
    if (!name || !src) {
      logErrorNoLine(
        `Expected load_images to be in the form "name1=src1 name2=src2 ...", ` +
          `but I saw "${arg}".`,
        true,
      );
      return;
    ...
    image.onerror = () => {
      logErrorNoLine(
        `An error occurred while loading image "${name}" from "${src}. ` +
          `Check the browser's developer console for details.`,
        true,
      );
...
function restoreLevel(
  ...
  if (debugSwitch.includes("load"))
    console.log(`restoreLevel()`, lev, snapCamera, resetTween, resetAutoTick);
  ...
  if (
    ...
    if (debugSwitch.includes("load"))
      console.log(
        `Restore level: from ${curLevel.width}x${curLevel.height} to ${lev.width}x${lev.height}`,
      );
    curLevel.width = lev.width;
    curLevel.height = lev.height;
...
function addUndoState(bak) {
  if (debugSwitch.includes("undo"))
    console.log(`addUndoState length=${backups.length} bak=`, bak);
  backups.push(bak);
  if (
    backups.length > 2 &&
...
function DoRestart(force) {
  ...
  tryPlayRestartSound();
  document.dispatchEvent(
    new CustomEvent("psplusLevelRestarted", { detail: curLevelNo }),
  );
...
function DoUndo(
  ...
  if (verbose_logging) {
    consolePrint(
      backups.length > 0 ? "--- undoing ---" : "Nothing to undo.",
      true,
    );
  }
  ...
  if (backups.length > 0) {
    ...
    if (debugSwitch.includes("undo"))
      console.log(`DoUndo length=${backups.length} torestore=`, torestore);
  ...
  for (let i = 0; i < state.sfx_MovementMasks[layer].length; i++) {
    ...
    if (sourceMask.get(fx.objId)) {
      ...
      if (movementMask.anyBitsInCommon(directionMask)) {
        // bug: two objects at location can cause false trigger
        if (verbose_logging)
          consolePrint(
            `Object "${state.idDict[fx.objId]}" has moved, playing seed "${fx.seed}".`,
          );
        if (fx.seed.startsWith("afx")) {
          const object = getObject(fx.objId);
          const move = getLayerMovement(movementMask, object.layer);
          const position = deltaPositionIndex(
            curLevel,
            positionIndex,
            dirMasksDelta[move][0],
            dirMasksDelta[move][1],
          );
    ...
    if (layerMovement !== 0) {
      ...
      if (thismoved) {
        if (state.metadata.tween_length && !dontModify) {
          ...
          var targetIndex =
            positionIndex + delta[1] + delta[0] * curLevel.height;
...
function Rule(rule) {
  ...
  for (var i = 0; i < this.patterns.length; i++) {
    this.cellRowMatches.push(
      this.generateCellRowMatchesFunction(
        this.patterns[i],
...
Rule.prototype.generateCellRowMatchesFunction = function (
  ...
  if (ellipsisCount === 0) {
    ...
    for (
      ;
      cellRow[cellIndex] !== ellipsisPattern && cellIndex < cr_l;
      cellIndex++
    ) {
      fn +=
        "&&cellRow[" +
    ...
    for (var idx = 1; idx < ellipsis_index_1; idx++) {
      fn +=
        "&&cellRow[" + idx + "].matches(i+" + idx + "*d, objects, movements)";
    }
    fn += ") {\n";
...
CellPattern.prototype.replace = function (rule, currentIndex) {
  ...
  if (rule.isRigid) {
    var rigidGroupIndex =
      state.groupNumber_to_RigidGroupIndex[rule.groupNumber];
    ...
    curRigidGroupIndexMask =
      curLevel.rigidGroupIndexMask[currentIndex] || new BitVec(STRIDE_MOV);
    curRigidMovementAppliedMask =
      curLevel.rigidMovementAppliedMask[currentIndex] || new BitVec(STRIDE_MOV);
...
function matchCellRow(
  ...
  if (
    !cellRowMask.bitsSetInArray(curLevel.mapCellContents.data) ||
    !cellRowMask_Movements.bitsSetInArray(
      curLevel.mapCellContents_Movements.data,
    )
  ) {
    return result;
  }
  ...
  if (isGlobal || state.metadata.local_radius === undefined) {
    ...
    xmin = Math.max(
      0,
      ((playerPositions[0] / curLevel.height) | 0) - localradius,
    );
    xmax = Math.min(
      curLevel.width,
      ((playerPositions[0] / curLevel.height) | 0) + localradius + 1,
    );
    ymin = Math.max(0, (playerPositions[0] % curLevel.height) - localradius);
    ymax = Math.min(
      curLevel.height,
      (playerPositions[0] % curLevel.height) + localradius + 1,
    );
  }
  ...
  if (horizontal) {
    for (var y = ymin; y < ymax; y++) {
      if (
        !cellRowMask.bitsSetInArray(curLevel.rowCellContents[y].data) ||
        !cellRowMask_Movements.bitsSetInArray(
          curLevel.rowCellContents_Movements[y].data,
        )
      ) {
        continue;
      }
    ...
    for (var x = xmin; x < xmax; x++) {
      if (
        !cellRowMask.bitsSetInArray(curLevel.colCellContents[x].data) ||
        !cellRowMask_Movements.bitsSetInArray(
          curLevel.colCellContents_Movements[x].data,
        )
      ) {
        continue;
      }
...
Rule.prototype.findMatches = function () {
  if (!this.ruleMask.bitsSetInArray(curLevel.mapCellContents.data)) return [];

  const d = curLevel.delta_index(this.direction);

  if (debugSwitch.includes("masks"))
    console.log(
      `Findmatches d=${d} dir=${this.direction} levobj=${curLevel.objects} levmov=${curLevel.movements}`,
    );
  var matches = [];
  ...
  for (
    ...
    if (this.ellipsisCount[cellRowIndex] === 1) {
      ...
      var match = matchCellRowWildCard(
        ...
        d,
        this.ellipsisCount[cellRowIndex],
      );
...
Rule.prototype.applyAt = function (level, tuple, check, delta) {
  ...
  for (
    ...
    var currentIndex =
      rule.ellipsisCount[cellRowIndex] > 0
        ? tuple[cellRowIndex][0]
        : tuple[cellRowIndex];
  ...
  if (verbose_logging && result) {
    ...
    const locations = cellIndexes
      .map((i) => `(${1 + (i % level.width)};${1 + ~~(i / level.width)})`)
      .join(", ");
  ...
  if (debugSwitch.includes("perf") && perfCounters.applied % 100 == 0)
    console.log(
      `Applied ${perfCounters.applied} rules in ${Date.now() - perfCounters.start}ms.`,
    );
  return result;
};
...
Rule.prototype.queueCommands = function () {
  ...
  for (var i = 0; i < commands.length; i++) {
    ...
    if (verbose_logging) {
      ...
      const logString = htmlColor(
        "green",
        `Rule ${htmlJump(this.lineNumber)} triggers command ${command[0]}.`,
      );
    ...
    if (
      state.metadata.runtime_metadata_twiddling &&
...
function twiddleMetadataExtras(resetAutoTick = true) {
  if (debugSwitch.includes("meta"))
    console.log(
      `twiddleMetaDataExtras resetAutoTick=${resetAutoTick} metadata:`,
      state.metadata,
    );
  autotickinterval = state.metadata.realtime_interval
    ? state.metadata.realtime_interval * 1000
    : 0;
  if (resetAutoTick || !state.metadata.realtime_interval) autotick = 0;
  againinterval = state.metadata.again_interval
    ? state.metadata.again_interval * 1000
    : 150;
  tweeninterval = state.metadata.tween_length
    ? Math.max(state.metadata.tween_length * 1000, 0)
    : 0;
  repeatinterval = state.metadata.key_repeat_interval
    ? state.metadata.key_repeat_interval * 1000
    : 200; // was 150, makes for key bounce
  animateinterval = state.metadata.animate_interval
    ? state.metadata.animate_interval * 1000
    : 250; // was 150, makes for key bounce

  const colorPalette = state.metadata.color_palette;
  state.bgcolor = state.metadata.background_color
    ? colorToHex(colorPalette, state.metadata.background_color)
    : "#000000";
  state.fgcolor = state.metadata.text_color
    ? colorToHex(colorPalette, state.metadata.text_color)
    : "#FFFFFF";
  state.author_color = state.metadata.author_color
    ? colorToHex(colorPalette, state.metadata.author_color)
    : state.fgcolor;
  state.title_color = state.metadata.title_color
    ? colorToHex(colorPalette, state.metadata.title_color)
    : state.fgcolor;
  state.keyhint_color = state.metadata.keyhint_color
    ? colorToHex(colorPalette, state.metadata.keyhint_color)
    : state.fgcolor;
}
  ...
  while (propagated) {
    ...
    for (var ruleIndex = 0; ruleIndex < ruleGroup.length; ruleIndex++) {
      ...
      if (rule.tryApply(curLevel)) {
        if (!rule.isOnce) propagated = true;
...
function applyRules(
  ...
  function findEnd(start) {
    ...
    if (start < rules.length) {
      ...
      let x = subroutines.findIndex(
        (s) => s.lineNumber > rules[start][0].lineNumber,
      );
      // find the rule group for that line number
      if (x != -1)
        result = rules.findIndex(
          (r) => r[0].lineNumber >= subroutines[x].lineNumber,
        );
    }
  ...
  for (let ruleGroupIndex = startRuleGroupindex; ruleGroupIndex < endIndex; ) {
    ...
    if (loopPropagated && loopPoint[ruleGroupIndex] >= 0) {
      ...
      if (gosubTarget >= 0) {
        ...
        if (verbose_logging)
          consolePrint(
            `Gosub to ${htmlJump(rules[gosubTarget][0].lineNumber)}`,
            true,
            rules[ruleGroupIndex][0].lineNumber,
          );
        ...
        if (debugSwitch.includes("gosub"))
          console.log(
            `gosub1 group:${ruleGroupIndex} line:${rules[ruleGroupIndex][0].lineNumber} endindex:${endIndex}`,
            gosubStack,
          );
      } else {
        ruleGroupIndex++;
        ...
        if (
          ruleGroupIndex == endIndex &&
          loopPropagated &&
          loopPoint[ruleGroupIndex] >= 0
        ) {
          if (checkLoop()) break;
        }
        ...
        while (ruleGroupIndex == endIndex && gosubStack.length > 0) {
          if (verbose_logging)
            consolePrint(
              `Return to ${htmlJump(rules[gosubStack.at(-1)][0].lineNumber)}`,
              true,
            );
          ...
          if (debugSwitch.includes("gosub"))
            console.log(
              `gosub2 group:${ruleGroupIndex} line:${rules[ruleGroupIndex][0].lineNumber} endindex:${endIndex}`,
              gosubStack,
            );
        }
      }
        ...
        if (!movementMask.iszero()) {
          ...
          for (var j = 0; j < level.layerCount; j++) {
            ...
            if (layerSection !== 0) {
              ...
              var rigidGroupIndex = rigidGroupIndexMask.getshiftor(
                MOV_MASK,
                MOV_BITS * j,
              );
      ...
      for (const fx of state.sfx_MovementFailureMasks) {
        if (cellMask.get(fx.objId)) {
          if (movementMask.anyBitsInCommon(fx.directionMask)) {
            ...
            if (verbose_logging)
              consolePrint(
                `Object "${state.idDict[object]}" can't move, playing seed "${seedsToPlay_CantMove[i]}"`,
              );
            if (fx.seed.startsWith("afx")) {
              const move = getLayerMovement(movementMask, object.layer);
...
function processLevelInput() {
  ...
  if (!input) return;
  if (verbose_logging) consolePrint(`Processing level input ${input}`);
...
function procInp(dir, dontDoWin, dontModify, bak, coord) {
  ...
  if (dir < dirNames.length) {
    ...
    // todo: reaction
    if ([0, 1, 2, 3, 4].includes(dir)) {
      // arrows plus action go to player
      playerPositions = startMovement(dirMasks[dirName]);
    } else if ([6, 7].includes(dir)) {
      // clicks go to object(s)
    ...
    if (verbose_logging) {
      ...
      if (dir === -1) {
        ...
        //  consolePrint('=======================');
        consolePrint(
          `Turn starts with input of ${dirName}.`,
          false,
          null,
          inspect_ID,
        );
    ...
    var alreadyResolved = [];
    if (dir != -1)
      // clear status line on user input
      statusText = "";
      ...
      if (shouldUndo) {
...

        {
          ...
          if (IDE) {
            ...
            var bannedLineNumbers = newBannedGroups.map(
              (rgi) => state.rules[rgi][0].lineNumber,
            );
            var ts = bannedLineNumbers.length > 1 ? "lines " : "line ";
            ts += bannedLineNumbers
              .map(
                (ln) =>
                  `<a onclick="jumpToLine(${ln});" href="javascript:void(0);">${ln}</a>`,
              )
              .join(", ");
            consolePrint(
              `Rigid movement application failed in rule-Group starting from ${ts}, and will be disabled in resimulation. Rolling back...`,
            );
          }
        ...
        if (verbose_logging) {
          var eof_idx =
            debug_visualisation_array[debugger_turnIndex].length + 1; //just need some number greater than any rule group
    ...
    if (curLevel.commandQueue.indexOf("cancel") >= 0) {
      if (verbose_logging) {
        ...
        var r =
          curLevel.commandQueueSourceRules[
            curLevel.commandQueue.indexOf("cancel")
          ];
    ...
    if (curLevel.commandQueue.indexOf("restart") >= 0) {
      if (verbose_logging && runrulesonlevelstart_phase) {
        var r =
          curLevel.commandQueueSourceRules[
            curLevel.commandQueue.indexOf("restart")
          ];
        logWarning(
    ...
    if (curLevel.commandQueue.indexOf("quit") >= 0 && !solving) {
      if (verbose_logging) {
        var r =
          curLevel.commandQueueSourceRules[
            curLevel.commandQueue.indexOf("quit")
          ];
    ...
    if (!winning && curLevel.commandQueue.indexOf("nosave") >= 0) {
      if (verbose_logging) {
        var r =
          curLevel.commandQueueSourceRules[
            curLevel.commandQueue.indexOf("nosave")
          ];
        consolePrintFromRule(
    ...
    if (
      (dontModify && curLevel.commandQueue.indexOf("win") >= 0) ||
      curLevel.commandQueue.indexOf("restart") >= 0
    ) {
      return true;
    }
    ...
    for (const entry of state.sfx_CreationMasks) {
      if (sfxCreateMask.get(entry.objId)) {
        // mask for objects created vs mask for sfx create event
        if (entry.seed.startsWith("afx")) {
          for (const fx of sfxCreateList) {
            if (fx.objId == entry.objId) {
              if (verbose_logging)
                consolePrint(
                  `Created object "${state.idDict[entry.objId]}", playing seed "${entry.seed}"`,
                );
              seedsToAnimate[fx.posIndex + "," + fx.objId] = {
                kind: "create",
                seed: entry.seed,
              };
            }
        ...
        if (entry.seed.startsWith("afx")) {
          for (const fx of sfxDestroyList) {
            if (fx.objId == entry.objId) {
              if (verbose_logging)
                consolePrint(
                  `Destroyed object "${state.idDict[entry.objId]}", playing seed "${entry.seed}"`,
                );
              seedsToAnimate[fx.posIndex + "," + fx.objId] = {
                kind: "destroy",
                seed: entry.seed,
    ...
    if (!winning) {
      if (curLevel.commandQueue.indexOf("checkpoint") >= 0) {
        if (verbose_logging) {
          var r =
            curLevel.commandQueueSourceRules[
              curLevel.commandQueue.indexOf("checkpoint")
            ];
          consolePrintFromRule(
      ...
      if (curLevel.commandQueue.indexOf("again") >= 0 && modified) {
        var r =
          curLevel.commandQueueSourceRules[
            curLevel.commandQueue.indexOf("again")
          ];
    ...
    curLevel.commandQueue = [];
    curLevel.commandQueueSourceRules = [];
    if (debugSwitch.includes("anim"))
      console.log(`Animate: ${JSON.stringify(seedsToAnimate)}`);
  }
...
function checkWin(dontDoWin) {
  ...
  if (state.winconditions.length > 0) {
    ...
    for (var wcIndex = 0; wcIndex < state.winconditions.length; wcIndex++) {
      ...
      const f1 = aggr1
        ? (c) => filter1.bitsSetInArray(c)
        : (c) => !filter1.bitsClearInArray(c);
      const f2 = aggr2
        ? (c) => filter2.bitsSetInArray(c)
...
function DoWin() {
  ...
  if (linkStack.length > 0) {
    // got here by link so go back there
    returnLink();
...
function nextLevel() {
  if (debugSwitch.includes("load"))
    console.log(`nextLevel()`, `curLevelNo=${curLevelNo}`);
  againing = false;
  messagetext = "";
  statusText = "";
...
function goToTitleScreen() {
  if (debugSwitch.includes("load"))
    console.log(
      `gotoTitleScreen() curlevelTarget=`,
      curlevelTarget,
      ` restartTarget=`,
      restartTarget,
    );
...
function initSmoothCamera() {
  if (debugSwitch.includes("meta"))
    console.log(`initSmoothCamera metadata:`, state.metadata);
...
function IsMouseGameInputEnabled() {
  return (
    state.metadata.mouse_left ||
    state.metadata.mouse_up ||
    state.metadata.mouse_drag ||
    state.metadata.mouse_clicks
  );
}
