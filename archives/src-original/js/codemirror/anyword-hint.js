...
(function (mod) {
  ...
  var PRELUDE_COMMAND_WORDS = [
    ...
    [
      "debug",
      "",
      "This outputs the compiled instructions whenever you build your file.",
    ],
    ["enable_pause", "", "Enable pause menu when Escape key is pressed."],
    [
      "flickscreen",
      "8x5",
      "Setting flickscreen divides each level into WxH grids, and zooms the camera in so that the player can only see one at a time",
    ],
    ...
    [
      "norepeat_action",
      "",
      "The action button will only respond to individual presses, and not auto-trigger when held down.",
    ],
    ...
    [
      "realtime_interval",
      "",
      "The number indicates how long each realtime frame should be.",
    ],
    [
      "require_player_movement",
      "",
      "If the player doesn't move, cancel the whole move.",
    ],
    [
      "run_rules_on_level_start",
      "",
      "Applies the rules once on level-load, before the player has moved",
    ],
    ...
    [
      "title",
      "My Amazing Puzzle Game",
      "The name of your game. Appears on the title screen.",
    ],
    [
      "throttle_movement",
      "",
      "For use in conjunction with realtime_interval - this stops you from moving crazy fast - repeated keypresses of the same movement direction will not increase your speed. This doesn't apply to the action button.",
    ],
    [
      "verbose_logging",
      "",
      "As you play the game, spits out information about all rules applied as you play, and also allows visual inspection of what exactly the rules do by hovering over them with your mouse (or tapping them on touchscreen).",
    ],
    [
      "zoomscreen",
      "WxH",
    ...
    [
      "mouse_left",
      "lmb",
      "Sets the object to place when pressing left mouse button.",
    ],
    [
      "mouse_drag",
      "drag",
      "Sets the object to place when moving the mouse while left mouse button is held.",
    ],
    [
      "mouse_up",
      "lmbup",
      "Sets the object to place when left mouse button is released.",
    ],
    [
      "mouse_right",
      "rmb",
      "Sets the object to place when pressing right mouse button.",
    ],
    [
      "mouse_rdrag",
      "rdrag",
      "Sets the object to place when moving the mouse while right mouse button is held.",
    ],
    [
      "mouse_rup",
      "rmbup",
      "Sets the object to place when right mouse button is released.",
    ],
    [
      "nokeyboard",
      "",
      "Allows the game to run without a Player object and movement.",
    ],
    [
      "smoothscreen",
      "[flick] WxH [IxJ] [S]",
      "Zooms the camera in to a WxH section of the map around the player and smoothly scrolls to follow the player. The optional argument IxJ (default: 1x1) specifies a boundary in which the player can move without moving the camera. The optional speed argument (default: 0.125) specifies how far the camera will move towards the player each frame as a fraction of the distance to the player. The optional flick keyword makes the camera move like flickscreen.",
    ],
    ...
    [
      "level_select",
      "",
      "Show level selection option on main screen (use with sections in levels).",
    ],
    [
      "continue_is_level_select",
      "",
      "Don't show separate option on main menu for level selection.",
    ],
    [
      "level_select_lock",
      "",
      "Level selection will have unsolved levels locked.",
    ],
    [
      "level_select_unlocked_ahead",
      "1",
      "Level selection will have N number of unsolved levels unlocked after last solved level.",
    ],
    [
      "level_select_solve_symbol",
      "x",
      "Symbol that will be used for solved levels in level select menu",
    ],
    ...
    [
      ...
      "",
      "Allows you to change some prelude flags during runtime using rules",
    ],
    [
      "runtime_metadata_twiddling_debug",
      "",
      "Prints in the console each time the metadata is twiddled",
    ],
    [
      "skip_title_screen",
      "",
      "Will skip the title screen when the game initializes, booting straight into the level or level select",
    ],
    [
      "tween_length",
      "0.05",
      "Will animate objects that are moving using LEFT, RIGHT, UP, and DOWN. Duration in seconds.",
    ],
    [
      "tween_easing",
      "linear",
      "How movement animations should lerp when using TWEEN_LENGTH, e.g. 'easeInQuad', 'easeOutCubic', etc.",
    ],
    [
      "tween_snap",
      "5",
      "With how much granularity to snap to the underlying sprite grid. Uses the SPRITE_SIZE by default.",
    ],
    [
      "message_text_align",
      "center",
      "Towards which side message text should align. left, center (default), or right.",
    ],
    [
      "text_controls",
      "arrow keys to move \\nX to action",
      "Overwrite keyboard/mouse control description on the title screen.",
    ],
    [
      "text_message_continue",
      "[x] or [space] to continue!",
      "Overwrite the text below messages that normally says 'X to continue'.",
    ],
    [
      "level_select_unlocked_rollover",
      "3",
      "An alternative to LEVEL_SELECT_UNLOCKED_AHEAD, which applies a strict maximum number of unsolved levels.",
    ],
    [
      "sitelock_origin_whitelist",
      "https://auroriax.github.io:1234",
      "Exported versions of the game will only run on sites with this origin.",
    ],
    [
      "sitelock_hostname_whitelist",
      "polyomino.com",
      "Exported versions of the game will only run on sites with this hostname.",
    ],
    [
      "allow_undo_level",
      "",
      "Allows a player to go back to just before where a link came from.",
    ],
    [
      "mouse_clicks",
      "",
      "Enables mouse clicks to be treated as movements in rules.",
    ],
    ...
    [
      "keyhint_color",
      "blue",
      "Controls the font color of lines giving hints about the keys to use.",
    ],
    [
      "author_color",
      "red",
    ...
    [
      "load_images",
      "my_sprites=https://[...]/sprites.png",
      "Loads an external image to act as a spritesheet for objects.",
    ],
    [
      "debug_switch",
      "<options>",
      "Enables debugging to console as set by <options>. Consult source code for available options.",
    ],
    //["auto_level_titles", "", ""],
    //["puzzlescript", "", ""],
    //["youtube", "", ""],
    ...
    switch (state.section) {
      case "objects": {
        if (state.objects_section == 3) {
          // not sure quite why 3 instead of 2
          candlists.push(COLOR_WORDS);
        }
        break;
      }
      case "legend": {
        var splits = lineToCursor
          .toLowerCase()
          .split(/[\p{Z}\s]/u)
          .filter(function (v) {
            return v !== "";
      ...
      case "sounds": {
        ...
        if (
          last_idx > 0 &&
          state.current_line_wip_array[last_idx] === "ERROR"
        ) {
      ...
      default: {
        ...
        if (
          ...
          if (linewords.length < 2) {
            ...
            linewords.length == 2 &&
            linewords[0].toLowerCase() == "color_palette"
          ) {
    ...
    if (addObjects) {
      ...
      for (var key in obs) {
        if (obs.hasOwnProperty(key)) {
          ...
          if (
            (!curWord || matchWord.lastIndexOf(curWord, 0) == 0) &&
            !Object.prototype.hasOwnProperty.call(seen, matchWord)
          ) {
            seen[matchWord] = true;
            const hint = state.original_case_names[w];
            if (hint)
              // no hint if object defined later than source line
              list.push({
                text: hint,
                extra: "",
                tag: "NAME",
