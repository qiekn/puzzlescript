  ...
  CodeMirror.defineExtension("uncomment", function (from, to, options) {
    ...
    lineComment: {
      for (var i = start; i <= end; ++i) {
        ...
        if (
          found_s > -1 &&
          !/comment/.test(self.getTokenTypeAt(Pos(i, found_s + 1)))
        )
          found_s = -1;
        if (found_s == -1 && nonWS.test(line)) break lineComment;
        if (found_s > -1 && nonWS.test(line.slice(0, found_s)))
          break lineComment;

        if (
          found_e > -1 &&
          !/comment/.test(self.getTokenTypeAt(Pos(i, found_e)))
        )
          found_e = -1;
        if (found_e == -1 && nonWS.test(line)) break lineComment;
        if (found_e > -1 && nonWS.test(line.slice(found_e + 1)))
          break lineComment;
      ...
      self.operation(function () {
        for (var i = start; i <= end; ++i) {
          ...
          if (line.slice(endPos_e - 1 - pad.length, endPos_e - 1) == pad)
            pos_e -= pad.length;
          if (line.slice(endPos_s, endPos_s + pad.length) == pad)
            endPos_s += pad.length;
          didSomething = true;
