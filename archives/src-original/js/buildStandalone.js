...
function buildStandalone(sourceCode) {
  function removeDiv(div) {
    ...
    if (start >= 0 && end >= 0) {
      htmlString =
        htmlString.substring(0, start) + htmlString.substring(end + 6);
    }
  }

  function patchGameContainer(top) {
    for (
      let start = htmlString.indexOf(".gameContainer");
      start != -1;
      start = htmlString.indexOf(".gameContainer", start + 1)
    ) {
      const end = htmlString.indexOf("}", start);
      if (
        start >= 0 &&
        end >= 0 &&
        htmlString.substring(start, end).indexOf(top) >= 0
      ) {
        const head = htmlString.substring(0, start);
        const tail = htmlString
          .substring(start)
          .replace(/top:.*?;/, "top:0;")
  ...
  const title = state.metadata.title
    ...
    : "PuzzleScript Next Game";

  var homepage = state.metadata.homepage
    ? state.metadata.homepage
    : "https://www.puzzlescript.net";
  if (!homepage.match(/^https?:\/\//)) homepage = "https://" + homepage; // todo: could do better URL validation here
  const homepage_stripped = escapeHtmlChars(
    homepage.replace(/^https?:\/\//, ""),
  );

  const background_color =
    "background_color" in state.metadata ? state.bgcolor : "black";
  ...
  if (exportOptions.includes("notitle")) removeDiv("class=title");
  if (exportOptions.includes("nofooter")) removeDiv("class=footer");
  if (exportOptions.includes("nopadding")) patchGameContainer("top:3");
