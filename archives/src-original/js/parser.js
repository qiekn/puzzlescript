...
const reg_commandwords =
  /^(afx[\w:=+-.]+|sfx\d+|cancel|checkpoint|restart|win|message|again|undo|nosave|quit|zoomscreen|flickscreen|smoothscreen|again_interval|realtime_interval|key_repeat_interval|noundo|norestart|background_color|text_color|goto|message_text_align|status|gosub|link|log|color_palette)$/i;
...
function htmlJump(lineNumber, clss) {
  return clss
    ? htmlClass(clss, htmlJump(lineNumber))
...
function logWarningNoLine(str, urgent, increaseErrorCount = false) {
  if (compiling || urgent) {
    ...
    if (errorStrings.indexOf(errorString) >= 0 && !urgent) {
      ...
      consolePrint(errorString, true);
      errorStrings.push(errorString);
      if (increaseErrorCount) errorCount++;
      if (errorCount > MAX_ERRORS_FOR_REAL) {
        TooManyErrors();
      }
    }
...
var codeMirrorFn = function () {
  ...
  const reg_equalsrow = /[\=]+/;
  const reg_soundevents =
    /^(sfx\d+|undo|restart|titlescreen|startgame|cancel|endgame|startlevel|endlevel|showmessage|closemessage|pausescreen)\b/i;

  const reg_loopmarker = /^(startloop|endloop)$/;
  const reg_ruledirectionindicators =
    /^(up|down|left|right|horizontal|vertical|orthogonal|late|rigid)$/;
  const reg_sounddirectionindicators =
    /^(up|down|left|right|horizontal|vertical|orthogonal)\b/i;
  ...
  const prelude_tables = [
    prelude_keywords,
    prelude_param_text,
    prelude_param_number,
    prelude_param_single,
    prelude_param_multi,
  ];
  const color_names = [
    "black",
  ...
  function expandSymbol(state, name, isand, cbError) {
    if (name in state.objects) return [name];
    for (const sym of state.legend_synonyms)
      if (sym[0] == name) return expandSymbol(state, sym[1], isand);
    for (const sym of state.legend_aggregates) {
      if (sym[0] == name) {
        if (!isand) cbError(name);
  ...
  function createAlias(state, alias, candname, lineno) {
    if (debugSwitch.includes("alias"))
      console.log(`Create '${alias}' as alias for '${candname}'`);
  ...
  class Lexer {
    ...
    matchComment() {
      while (true) {
        ...
        if (token) this.pushToken(token, "comment");
        else break;
      }
    }
  ...
  function matchComment(stream, state) {
    stream.match(/\s*/);
    if (stream.eol()) return state.commentLevel > 0 ? "" : null;
    ...
    if (state.commentStyle == "//") {
      if (stream.match("//", false)) return stream.match(/.*/)[0];
    }
    // handle () comments
    if (state.commentLevel == 0 && stream.peek() != "(") return null;
    const pos = stream.pos;
    do {
      stream.match(/[^\(\)]*/);
      if (stream.eol()) break;
      if (stream.match("(")) state.commentLevel++;
      else if (stream.match(")")) state.commentLevel--;
  ...
  function blankLineHandle(state) {
    if (state.section == "levels") {
      ...
      if (toplevel && toplevel.length > 0) state.levels.push([]);
  ...
  function parseSection(stream, state) {
    ...
    if (
      !(section && sectionNames.includes(section) && checkEol(stream, state))
    ) {
      pushBack(stream);
      return false;
    }
    ...
    if (state.section === "") {
      ...
      logError(
        `Unexpected end of section, MAPPING definition incomplete.`,
        state.lineNumber,
      );
  ...
  function parsePrelude(stream, state) {
    ...
    if ((value = getTokens())) setState(state, value);
    if (!lexer.matchEol()) lexer.pushToken(lexer.matchAll(), "ERROR");
    return lexer.tokens;
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(/^[a-z_]+/i, true))) {
        if (state.metadata_lines[ident]) {
          ...
          logWarning(
            `You've already defined a "${errorCase(token)}" in the prelude on line ${htmlJump(otherline)}.`,
            state.lineNumber,
          );
          ...
          logWarning(
            `Option ${errorCase(token)} is not implemented, but may be in the future. Let me know if you really need it.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
          return;
          ...
          while (!lexer.matchEol()) {
            if ((token = lexer.match(/^\S+/, true))) {
              kind =
                token in colorPalettes.arnecolors
                  ? "COLOR COLOR-" + errorCase(token)
                  : token === "transparent"
                    ? "COLOR FADECOLOR"
                    : token.match(/^#[0-9a-fA-F]+$/)
                      ? "MULTICOLOR" + token
                      : "METADATATEXT";
          ...
          logWarning(
            `Prelude option "${errorCase(token)}" is not one I know, so I'm going to ignore it. Hope that works for you.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        }
      ...
      if ((token = lexer.matchToken())) {
        logError(
          `Unrecognised stuff in the prelude: "${errorCase(token)}".`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
      } else if (ident) {
    ...
    function checkArguments(ident, args) {
      ...
      if (prelude_keywords.includes(ident)) {
        if (args.length > 1)
          logError(
            `Prelude option "${errorCase(ident)}" doesn't take any parameters, but you went and gave it "${args.join()}".`,
            state.lineNumber,
          );
        ...
        if (args.length != 1 || parseFloat(args[0]) == NaN)
          logError(
            `Prelude option "${errorCase(ident)}" requires one numeric argument.`,
            state.lineNumber,
          );
        else value = [ident, parseFloat(args[0])];
      } else if (
        prelude_param_single.includes(ident) ||
        prelude_param_text.includes(ident)
      ) {
        if (!(args.length == 1 || ident.startsWith("mouse")))
          logError(
            `Prelude option "${errorCase(ident)}" requires exactly one argument, but you gave it ${args.length}.`,
            state.lineNumber,
          );
        else if (ident.includes("_color") && !isColor(args[0]))
          logError(
            `Prelude option "${errorCase(ident)}" in incorrect format - found ${args[0]}, but I expect a color name (like 'pink') or hex-formatted color (like '#1412FA'). A default will be used.`,
            state.lineNumber,
          );
        ...
        if (args.length < 1)
          logError(
            `Prelude option "${errorCase(ident)}" has no arguments, but it needs at least one.`,
            state.lineNumber,
          );
    ...
    function setState(state, value) {
      ...
      if (ident == "case_sensitive") {
        ...
        caseSensitive = true;
        if (
          Object.keys(state.metadata).some((k) =>
            prelude_param_text.includes(k),
          )
        )
          logWarningNoLine(
            "Please make sure that CASE_SENSITIVE comes before any case sensitive prelude setting.",
            false,
            false,
          );
        ...
        ident == "mouse_clicks" &&
        !directions_table.includes(mouse_clicks_table[0])
      ) {
        ...
        if (
          ...
          logError(
            `Prelude optiton "${errorCase(ident)}" expects an argument of 1 or 2 numbers, like 10 or 4x7, not ${value[1]}.`,
            state.lineNumber,
          );
        else {
          state.sprite_size = args[0];
          if (args.length == 2) state.cell_height = args[1];
        }
        ...
        logWarning(
          `Setting "${errorCase(ident)}" is an experimental Pattern:Script feature. Do not use.`,
          state.lineNumber,
        );
        return;
      }
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(reg_name, !state.case_sensitive))) {
        ...
        if (
          ...
          token.match(/^(player|background)$/i)
        ) {
          logError(
            `You cannot define a tag called "${errorCase(token)}" because the name is already in use.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        ...
        logError(
          `Expected a name for a new tag, but found "${errorCase(token)}".`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      if ((token = lexer.match(/^=/, true))) {
        ...
        token = lexer.matchToken();
        logError(
          `Expected an equals sign "=" after the tag name but got "${errorCase(token)}".`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      while (true) {
        ...
        if ((token = lexer.match(reg_name, !state.case_sensitive))) {
          ...
          token = lexer.matchToken();
          logError(
            `Expected a name for a new tag member, but found "${errorCase(token)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
          return;
  ...
  function parseMappingsLine1(stream, state) {
    ...
    if (getTokens()) setState();
    if (!lexer.matchEolSemi()) lexer.pushToken(lexer.matchAll(), "ERROR");
    return lexer.tokens;
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(reg_name, !state.case_sensitive))) {
        symbols.lhs = token;
        if (!(isDeclaredAs(state, token) == "tag")) {
          // todo: propobj
          logError(
            `Expected a TAG name but "${errorCase(token)}" is not one.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        } else {
        ...
        token = lexer.matchToken();
        logError(
          `Expected a TAG name, but found "${errorCase(token)}" instead.`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      if ((token = lexer.match(/^=>/))) {
        ...
        token = lexer.matchToken();
        logError(
          `Expected an arrow sign "=>" but got "${errorCase(token)}" instead.`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      if ((token = lexer.match(reg_name, !state.case_sensitive))) {
        ...
        if (wordAlreadyDeclared(state, token)) {
          logError(
            `You cannot define a mapping called "${errorCase(token)}" because the name is already in use.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        } else {
        ...
        token = lexer.matchToken();
        logError(
          `Expected a name for a mapping, but got "${errorCase(token)}" instead.`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      if ((token = lexer.matchToken())) {
        logError(
          `Unrecognised stuff in a mapping: "${errorCase(token)}".`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
  ...
  function parseMappingsLine2(stream, state) {
    ...
    function getTokens() {
      ...
      while ((token = lexer.match(reg_name, !state.case_sensitive))) {
        if (!state.tags[mapping.fromKey].includes(token)) {
          // todo: prop
          logError(
            `The name "${errorCase(token)}" needs to be defined by "${errorCase(mapping.fromKey)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        } else {
      ...
      while ((token = lexer.match(reg_name, !state.case_sensitive))) {
        if (!token) {
          ...
          logError(
            `The name "${errorCase(token)}" is a very bad thing.`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        } else {
    ...
    function setState() {
      if (
        !(
          symbols.lhs.length == symbols.rhs.length &&
          new Set(symbols.lhs).size == symbols.lhs.length &&
          new Set(symbols.lhs).size == symbols.rhs.length
        )
      )
        logError(
          `Oops! You need the same number of symbols on both sides, and no duplicates.`,
          state.lineNumber,
        );
      else {
  ...
  function parseObjectName(stream, state) {
    ...
    // build a list of tokens and kinds
    function getTokens() {
      if (state.case_sensitive) stream.string = state.mixedCase;
      lexer.matchComment();
      ...
      while (true) {
        ...
        if (symbols.candname && lexer.check(reg_objmodi)) {
          ...
          (token =
            lexer.match(reg_objectname, !state.case_sensitive) ||
            (symbols.candname && lexer.matchObjectGlyph(!state.case_sensitive)))
        ) {
          if (state.legend_synonyms.some((s) => s[0] == token)) {
            logError(
              `Name "${errorCase(token)}" already in use.`,
              state.lineNumber,
            );
          } else if (state.objects[token] && !state.objects[token].canRedef) {
            logError(
              `Object "${errorCase(token)}" defined multiple times.`,
              state.lineNumber,
            );
          } else {
            if (keyword_array.includes(token))
              logWarning(
                `You named an object "${errorCase(token)}", but this is a keyword. Don't do that!`,
                state.lineNumber,
              );
            kind = "NAME";
          ...
          logError(
            `Invalid object name in OBJECT section: "${errorCase(token)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
    ...
    function setState() {
      ...
      const newobj = state.objects[candname] || {
        // doc: array of objects indexed by name
        lineNumber: state.lineNumber,
      };
  ...
  function parseObjectColors(stream, state) {
    ...
    if (getTokens()) state.objects[state.objects_candname].colors = colors;
    return lexer.tokens;
      ...
      while (!lexer.matchEolSemi()) {
        ...
        if ((token = lexer.match(/^[\w]+/, true))) {
          if (color_names.includes(token)) {
            ...
            kind =
              token in colorPalettes.arnecolors
                ? `COLOR COLOR-${token.toUpperCase()}`
                : token === "transparent"
                  ? "COLOR FADECOLOR"
                  : `MULTICOLOR${token}`;
          } else
            logWarning(
              `Invalid color in object section: "${errorCase(token)}".`,
              state.lineNumber,
            );
        } else if (
          (token = lexer.match(/^#(([0-9a-f]{2}){3,4}|[0-9a-f]{3,4})/i))
        ) {
          colors.push(token);
          kind = `MULTICOLOR${token}`;
        } else if ((token = lexer.matchToken())) {
          logError(
            `Was looking for color for object "${errorCase(state.objects_candname)}", got "${errorCase(token)}" instead.`,
            state.lineNumber,
          );
  ...
  function parseObjectSprite(stream, state) {
    ...
    if (getTokens()) {
      if (values.text) obj.spritetext = values.text;
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(/^text:/i, false))) {
        ...
        const kind =
          obj.colors.length == 1
            ? `COLOR COLOR-${obj.colors[0].toUpperCase()}`
            : "ERROR";
        lexer.pushToken(token, kind);
        values.text = token;
        state.objects_section = 0;
      ...
      if ((token = lexer.match(/^\{[^}]*\}/, false))) {
        logError(
          `This ${token} looks rather like a piece of JSON. Did you maybe leave out "CANVAS:"?`,
          state.lineNumber,
        );
        lexer.pushToken(token, kind);
      ...
      while (!stream.eol()) {
        ...
        if (token == ".") kind = "COLOR FADECOLOR";
          ...
          value =
            token <= "9" ? +token : 10 + token.toLowerCase().charCodeAt(0) - 97; // letter 'a'
          if (!obj.colors[value])
            logError(
              `Trying to access color number ${value + 1} from the color palette of sprite ${state.objects_candname}, but there are only ${obj.colors.length} defined in it."`,
              state.lineNumber,
            );
          else
            kind = "COLOR BOLDCOLOR COLOR-" + obj.colors[value].toUpperCase();
        } else
          logError(
            `Invalid character "${errorCase(token)}" in sprite for ${state.objects_candname}`,
            state.lineNumber,
          );
        lexer.pushToken(token, kind);
        values.push(value);
  ...
  function parseObjectVector(stream, state) {
    ...
    if (getTokens()) obj.vector.data = (obj.vector.data || []).concat(values);
    if (!lexer.matchEol()) lexer.pushToken(lexer.matchAll(), "ERROR");
    return lexer.tokens;
    ...
    function getTokens() {
      ...
      if (obj.vector.type == "canvas") {
        ...
        if (kind == "ERROR")
          logError(
            `I was looking for some valid JSON (in curly braces) but found this instead: "${token}."`,
            state.lineNumber,
          );
        return kind != `ERROR`;
  ...
  function parseObjectTransforms(stream, state) {
    ...
    function isValidDirection(arg) {
      ...
      return tagvalues &&
        tagvalues.every((v) => clockwiseDirections.includes(v))
        ? arg
        : mappings &&
            mappings.values &&
            mappings.values.every((v) => clockwiseDirections.includes(v))
          ? arg
          : clockwiseDirections.includes(arg)
            ? arg
            : null;
    }
    ...
    function isValidNumeric(arg) {
      ...
      if (tagvalues && tagvalues.every((v) => !isNaN(parseFloat(v))))
        return arg;
      if (
        mappings &&
        mappings.values &&
        mappings.values.every((v) => !isNaN(parseFloat(v)))
      )
        return arg;
      ...
      while (!lexer.matchEolSemi()) {
        ...
        if ((token = lexer.match(/^copy:/i))) {
          if (symbols.cloneSprite)
            logError(
              `You already assigned a sprite source for "${errorCase(symbols.candname)}", you can't have more than one!`,
              state.lineNumber,
            );
          else kind = "KEYWORD";
          lexer.pushToken(token, kind);
          ...
          if (!(token = lexer.match(reg_objectname, !state.case_sensitive)))
            ...
            logError(
              `You attempted to set the sprite "${errorCase(token)}" to copy from itself! Please don't.`,
              state.lineNumber,
            );
          ...
          if (dir == null)
            logError(
              `Flip requires a direction argument, but you gave it ${errorCase(token)}.`,
              state.lineNumber,
            );
          else {
          ...
          if (!(args.length <= 2 && dir != null && amt != null))
            logError(
              `Shift requires a direction argument and optionally an amount, but you gave it ${errorCase(token)}.`,
              state.lineNumber,
            );
          else {
          ...
          if (!(args.length == 2 && dir != null && amt != null))
            logError(
              `Translate requires two arguments, a direction and an amount, not ${errorCase(token)}.`,
              state.lineNumber,
            );
          else {
          ...
          if (!(args.length <= 2 && dir1 && dir2))
            logError(
              `For rot: you need 1 or 2 direction or tag arguments, but you gave it ${token ? errorCase(token) : "neither"}.`,
              state.lineNumber,
            );
          else {
          ...
          if ((token = lexer.match(/^[0-9,]+/, true))) {
            ...
            if (!(parts.length >= 1 && parts.length <= 2))
              logError(
                `Canvas size has to be specified as a number or number,number, not ${errorCase(token)}.`,
                state.lineNumber,
              );
            else {
          ...
          logError(
            `Invalid token in OBJECT modifier section: "${errorCase(token)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        } else throw "obj-modi";
  ...
  function parseSoundLine(stream, state) {
    ...
    if (getTokens()) state.sounds.push(...rows);
    if (!lexer.matchEol()) lexer.pushToken(lexer.matchAll(), "ERROR");
    return lexer.tokens;
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(reg_soundevents, true))) {
        ...
        if (tsounds) {
          rows.push(
            ...tsounds.map((s) => ["SOUNDEVENT", tevent, s, state.lineNumber]),
          );
          return true;
        } else {
          logError(
        ...
        if (!tobjects) {
          ...
          logError(
            `Found "${errorCase(token)}", which looks like an object but ${undef[0] ? errorCase(undef[0]) : "it"} is not declared anywhere.`,
            state.lineNumber,
          );
        } else {
          lexer.pushToken(token, "NAME");
          ...
          if ((token = lexer.match(/^[a-z]+/i, true))) {
            if (
              soundverbs_directional.includes(token) ||
              soundverbs_movement.includes(token) ||
              soundverbs_other.includes(token)
            ) {
          ...
          if (!tverb) {
            ...
            if (tsounds) {
              ...
              logError(
                `Ah I was expecting a ${msg} after ${tverb}, but I don't know what to make of "${errorCase(token)}".`,
                state.lineNumber,
              );
            }
          }
    ...
    function parseSoundSeedsTail() {
      ...
      let token = null;
      while (
        (token = lexer.match(/^(\d+(:[\d.]+)?|afx:[\w:=+-.]+)\b/i, true))
      ) {
      ...
      if ((token = lexer.matchToken())) {
        logError(
          `I wasn't expecting anything after the sound declaration ${tsounds.at(-1)} on this line, so I don't know what to do with "${errorCase(token)}" here.`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return null;
  ...
  function parseLegendLine(stream, state) {
    ...
    function getTokens() {
      ...
      if (
        (token =
          lexer.match(reg_objectname, !state.case_sensitive) ||
          lexer.matchObjectGlyph(!state.case_sensitive))
      ) {
        symbols.newname = token;
        const defname = wordAlreadyDeclared(state, token);
        if (defname)
          logError(
            `Name "${errorCase(token)}" already in use (on line ${htmlJump(defname.lineNumber)})`,
            state.lineNumber,
          );
      ...
      while (true) {
        ...
        if (
          ...
          if (!tobjects) {
            const undef = getObjectUndefs(state, token);
            logError(
              `You're talking about "${errorCase(token)}" but ${undef[0] ? errorCase(undef[0]) : "it"} is not defined anywhere.`,
              state.lineNumber,
            );
          } else if (token == symbols.newname)
            logError(
              `You can't define object "${errorCase(token)}" in terms of itself!`,
              state.lineNumber,
            );
          else {
            if (names.includes(token))
              logWarning(
                `You're repeating the object "${errorCase(token)}" here multiple times on the RHS.  This makes no sense.  Don't do that.`,
                state.lineNumber,
              );
            names.push(...tobjects);
        ...
        if (kind != "NAME") {
          token = lexer.matchToken();
          logError(
            `Something bad's happening in the LEGEND around ${token}`,
            state.lineNumber,
          );
          return;
        }
        ...
        if ((token = lexer.match(/^(and|or)\b/i, true))) {
          ...
          logError(
            `AND or OR expected, found ${errorCase(token)}`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
          return;
    ...
    function setState() {
      if (names.length == 1) {
        ...
        const newlegend = [
          symbols.newname,
          ...names.flatMap((n) =>
            expandSymbol(state, n, false, () =>
              logError(
                "Cannot define a property (something defined in terms of 'or') in terms of aggregates (something that uses 'and').",
                state.lineNumber,
              ),
            ),
          ),
  ...
  function parseCollisionLayer(stream, state) {
      ...
      while (true) {
        if ((token = lexer.match(/^--[-^v<>-|]*/))) {
          ...
          if (token == "background" && idents.length != 0)
            ...
            const undef = getObjectUndefs(state, token);
            logError(
              `Cannot add "${errorCase(token)}" to a collision layer, ${undef[0] ? errorCase(undef[0]) : "it"} has not been declared.`,
              state.lineNumber,
            );
          } else {
            if (idents.includes(token))
              logWarning(
                `Object "${errorCase(token)}" included explicitly multiple times in the same layer. Don't do that innit.`,
                state.lineNumber,
              );
          ...
          token = lexer.match(reg_notcommentstart);
          logError(
            `Object name expected, found ${errorCase(token)}`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
          return;
    ...
    function setState() {
      ...
      function addToLayer(key, objects) {
        if (keylayers[key]) keylayers[key].layer.push(...objects);
      ...
      for (const prefix of prefixes) {
        if (
          !(
            state.tags[prefix] ||
            state.legend_properties.find((s) => s[0] == prefix)
          )
        ) {
          logError(
            `A layer prefix must be a tag or property but ${prefix} is not.`,
            state.lineNumber,
          );
          return;
        }
      ...
      for (const ident of rhs) {
        ...
        const newobjs = idents
          .map((i) =>
            expandSymbol(state, i, false, (n) =>
              logError(
                `"${n}" is an aggregate (defined using "and"), and cannot be added to a single layer because its constituent objects must be able to coexist.`,
                state.lineNumber,
              ),
            ),
          )
        ...
        state.collisionLayers.forEach((layer, layerno) => {
          for (const obj of newobjs) {
            if (layer.includes(obj)) dups.add(layerno + 1);
          }
        });
        if (dups.size != 0) {
          const joins =
          ...
          logWarning(
            `Object "${errorCase(ident)}" included in multiple collision layers ( layers ${joins} ). You should fix this!`,
            state.lineNumber,
          );
        }
  ...
  function parseWinCondition(stream, state) {
    ...
    function getTokens() {
      ...
      if ((token = lexer.match(/^(all|any|no|some)\b/i, true))) {
        ...
        token = lexer.match(reg_notcommentstart);
        logError(
          `Expecting the start of a win condition ("ALL","SOME","NO") but got "${errorCase(token)}".`,
          state.lineNumber,
        );
        lexer.pushToken(token, "ERROR");
        return;
      ...
      if (!lexer.matchEol()) {
        if ((token = lexer.match(/^(on)\b/i, true))) {
          ...
          token = lexer.matchToken();
          logError(
            `Expecting the word "ON" but got "${errorCase(token)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
          return;
        ...
        if (!lexer.matchEol()) {
          token = lexer.matchToken();
          logError(
            `Error in win condition: I don't know what to do with "${errorCase(token)}".`,
            state.lineNumber,
          );
          lexer.pushToken(token, "ERROR");
        }
    ...
    function getIdent() {
      ...
      if (
        ...
        let kind = "ERROR";
        if (
          !(wordAlreadyDeclared(state, token) || createObjectRef(state, token))
        )
          logError(
            `Error in win condition: "${errorCase(token)}" is not a valid object name.`,
            state.lineNumber,
          );
        else {
          names.push(token);
  ...
  function parseLevel(stream, state) {
    ...
    function getTokens() {
      ...
      if (
        (token = lexer.match(
          /^(goto|level|link|message|section|title|input)/i,
          true,
        ))
      ) {
        // allow omision of whitespace (with no warning!)
        symbols.start = token;
        ...
        if (token == "link") {
          if (!(token = lexer.match(reg_objectname, !state.case_sensitive)))
            logError(
              `LINK needs an object to know where to put the link.`,
              state.lineNumber,
            );
          else if (!wordAlreadyDeclared(state, token))
            logError(
              `LINK object "${errorCase(token)}" not found, it needs to be already defined.`,
              state.lineNumber,
            );
          else {
            // @@
            ...
            if (!(objects && objects.length == 1))
              logError(
                `LINK object "${errorCase(token)}" only works with a simple object or an alias, not something created with AND or OR.`,
                state.lineNumber,
              );
            {
        ...
        while (true) {
          if (state.commentStyle == "()" && lexer.peek() == "(") break;
          if ((token = lexer.match(/^\S/, !state.case_sensitive))) {
            symbols.gridline += token;
            const kind = state.abbrevNames.includes(token) ? "LEVEL" : "ERROR";
            if (kind == "ERROR")
              logError(
                `Key "${errorCase(token)}" not found. Do you need to add it to the legend, or define a new object?`,
                state.lineNumber,
              );
            lexer.pushToken(token, kind);
          } else break;
  ...
  return {
    ...
    token: function (stream, state) {
      ...
      if (
        state.current_line_wip_array.length > 0 &&
        !["rules"].includes(state.section)
      ) {
        return flushToken();
      }
      ...
      if (state.commentStyle === "()" && stream.match(")")) {
        ...
        logWarning(
          "You're trying to use the wrong type of comment here.[This is highly suspicious; you probably want to fix it.]",
          state.lineNumber,
        );
        stream.skipToEnd();
        return "ERROR";
      ...
      if (state.line_should_end && !stream.eol()) {
        logError(
          "Only comments should go after " +
            state.line_should_end_because +
            " on a line.",
          state.lineNumber,
        );
        stream.skipToEnd();
      ...
      if (sol && stream.match(reg_equalsrow, true)) {
        // todo: not if we're in a level
        state.line_should_end = true;
        state.line_should_end_because = "a bunch of equals signs ('===')";
        return "EQUALSBIT";
      }

      if (sol && parseSection(stream, state)) return "HEADER";
      ...
      switch (state.section) {
        ...
        case "objects": {
          ...
          if (
            sol &&
            (state.objects_section == 3 || state.objects_section == 4)
          ) {
            // no blank line: criterion for end sprite: <= 10 colours, line matches object and not transform
            if (
              state.objects[state.objects_candname].colors.length <= 10 &&
          ...
          switch (state.objects_section) {
            case 1: {
              state.current_line_wip_array.push(
                ...parseObjectName(stream, state),
              );
              if (state.objects_candname) {
                if (stream.match(reg_objmodi, false))
                  state.current_line_wip_array.push(
                    ...parseObjectTransforms(stream, state),
                  );
            ...
            case 2:
              ...
              if (
                ...
                state.current_line_wip_array.push(
                  ...parseObjectColors(stream, state),
                );
                return flushToken();
              } // else fall through
            case 3:
        ...
        case "rules": {
          if (sol) {
            ...
            const rule = stream.string
              .substring(stream.pos)
              .match(reg_notcommentstart)[0];
          ...
          if (ch === "[" || ch === "|" || ch === "]" || ch === "+") {
            ...
            if (state.tokenIndex === 0 && reg_loopmarker.exec(m)) {
              ...
              (state.tokenIndex === 1 && directions_table.includes(m)) ||
              Object.hasOwn(state.tags, m)
            ) {
              stream.match(/[\p{Z}\s]*/u, true);
              return "DIRECTION";
            } else {
              if (state.names.indexOf(m) >= 0) {
                ...
                m.match(reg_objectnamerel) &&
                (wordAlreadyDeclared(state, m) || createObjectRef(state, m))
              ) {
                return "NAME";
              }
