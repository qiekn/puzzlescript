...
function createSprite(name, spritegrid, colors, size, height) {
  ...
  canvas.width =
    spritegrid.reduce((acc, row) => Math.max(acc, row.length), 0) * pixelSize;
  ...
  function addInstr(json) {
    for (const instr of json) {
      try {
        for (let [key, value] of Object.entries(instr)) {
          if (key === "!include") {
            ...
            if (include) {
              ...
              logErrorNoLine(
                `Something called "${value}" you wanted to include does not seem to exist.`,
              );
            }
            ...
            if (key === "drawImage") {
              ...
              if (imageName === undefined) {
                logErrorNoLine(
                  `This call to "drawImage" needs to have at least one argument.`,
                  true,
                );
                continue;
              }
              ...
              if (!image) {
                logErrorNoLine(
                  `Could not find or load an image called "${imageName}".`,
                  true,
                );
                continue;
              }
        ...
        console.log(error);
        logErrorNoLine(
          `Oops! Looks like there's something wrong with this bit of JSON: "${JSON.stringify(instr)}"`,
          true,
        );
        logErrorNoLine(
          `The system returned this error message: ${error}`,
          true,
        );
      }
    }
...
function drawTextWithFont(ctx, text, color, x, y, height) {
  const fontSize = state.metadata.font_size
    ? Math.max(0, parseFloat(state.metadata.font_size))
    : 1;
  const pixelsize = height * fontSize;
  const fontname =
    state.metadata.custom_font && loadedCustomFont
      ? "PuzzleCustomFont"
      : "Monospace";
...
function regenText() {
  ...
  if (debugSwitch.includes("redraw"))
    console.log(
      `regenText sz=${textsheetSize} fk=${fontKeys.length} textsheetCanvas=`,
      textsheetCanvas,
    );
}
...
function createVectorSprite(name, vector) {
  const canvas =
    vector.type === "canvas"
      ? createCanvasSprite(name, vector)
      : vector.type === "svg"
        ? createSvgSprite(name, vector)
        : null;
  return canvas;
}
...
function regenSpriteImages() {
  ...
  if (IDE === true) {
    textImages["editor_s"] = createSprite(
      "chars",
      editor_s_grille,
      undefined,
      5,
  ...
  objectSprites.forEach((s, i) => {
    if (s) {
      spriteImages[i] = s.text
        ? createTextSprite("t" + i.toString(), s.text, s.colors, s.scale)
        : s.vector
          ? createVectorSprite("v" + i.toString(), s.vector)
          : createSprite(
              i.toString(),
              s.dat,
              s.colors,
    ...
    if (state.objects.hasOwnProperty(n)) {
      ...
      objectSprites[object.id] = {
        ...
        colors: object.colors.map((c) =>
          colorToHex(state.metadata.color_palette, c),
        ), // in case we twiddled colours
...
function redraw() {
  ...
  if (debugSwitch.includes("redraw"))
    console.log(`Redraw: ${JSON.stringify(perfCounters)}`);

  if (textMode) redrawTextMode();
  ...
  if (state.metadata.custom_font === undefined || !loadedCustomFont) {
    ...
    for (let j = 0; j < TITLE_HEIGHT; j++) {
      drawTextWithFont(
        ...
        titleImage[j],
        lineColor(j),
        xoffset + (TITLE_WIDTH / 2 + 0.5) * cellwidth,
...
function redrawCellGrid(curlevel) {
  ...
  if (diffToVisualize !== null) {
    ...
    curlevel.rigidMovementAppliedMask =
      diffToVisualize.rigidMovementAppliedMask;
  }
  ctx.restore();
  ctx.fillStyle = state.bgcolor;
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  if (levelEditorOpened) {
    ...
    if (playerPositions.length > 0) {
      ...
      minMaxIJ = [
        mini,
        minj,
        Math.min(mini + screenwidth, curlevel.width),
        Math.min(minj + screenheight, curlevel.height),
      ];
    ...
    if (playerPositions.length > 0) {
      ...
      const minj = Math.max(
        Math.min(py - ((screenheight / 2) | 0), curlevel.height - screenheight),
        0,
      );
      minMaxIJ = [
        mini,
  ...
  if (debugSwitch.includes("redraw"))
    console.log(
      `redrawCell canvas=${canvas.width},${canvas.height} screen=${screenwidth},${screenheight} cell=${cellwidth},${cellheight}`,
      minMaxIJ,
    );
      ...
      const offs = {
        ...
        y:
          ...
          (obj.vector
            ...
            : (state.cell_height || state.sprite_size) -
              obj.spritematrix.length),
      };
      return {
        x:
          xoffset +
          (ij.x - this.minMax[0] - cameraOffset.x) * cellwidth +
          offs.x * ~~(cellwidth / state.sprite_size),
        y:
          yoffset +
          (ij.y - this.minMax[1] - cameraOffset.y) * cellheight +
          offs.y * ~~(cellheight / (state.cell_height || state.sprite_size)),
      };
    }
  }
  ...
  const animTween = doAfxAnimate
    ? 1 - clamp(tweentimer / animateinterval, 0, 1)
    : 0; // range 1 => 0
  ...
  if (isAnimating && debugSwitch.includes("tween"))
    console.log(
      `tween moveTween=${moveTween} animTween=${animTween} seedsToAnimate=`,
      seedsToAnimate,
  ...
  if (state.metadata.status_line)
    drawTextWithFont(
      ...
      statusText,
      state.fgcolor,
      xoffset + (screenwidth * cellwidth) / 2,
      yoffset + screenheight * cellheight + textcellheight * 0.6,
      textcellheight * 0.6,
    );

  if (diffToVisualize) drawDiffToVisualize();
  if (smoothscreen) {
    if (state.metadata.smoothscreen.debug) drawSmoothScreenDebug(ctx);
    ctx.restore();
  }

  if (levelEditorOpened) drawEditorIcons(minMaxIJ[0], minMaxIJ[1]);
  ...
  function drawObjects(render) {
    ...
    for (const group of state.collisionLayerGroups) {
      for (const posindex of render.getPosIndexes(group)) {
        ...
        for (
          let k = group.firstObjectNo;
          k < group.firstObjectNo + group.numObjects;
          ++k
        ) {
          const animate =
            animTween > 0 ? seedsToAnimate[posindex + "," + k] : null;
          if (posmask.get(k) || animate) {
            const obj = state.objects[state.idDict[k]];
            if (showLayers && obj.layer != showLayerNo) continue;
            ...
            const mt =
              doMoveTweens && getTweening(moveTween, drawpos, obj, posindex);
            if (mt) {
              [drawpos.x, drawpos.y, ctx.globalAlpha] = mt;
            } else {
              if (animate)
                params = calcAnimate(
                  animate.seed.split(":").slice(1),
                  animate.kind,
                  animate.dir,
                  params,
                  animTween,
  ...
  function calcAnimate(afx, kind, dir, params, tween) {
    const tween_table = {
      move: { slide: [-1, 0], zoom: [0, 1], turn: [0, 0.25], fade: [0, 1] },
      cant: {
        slide: [0, 0.2, 0],
        zoom: [1, 1.2, 1],
        turn: [0, 0.1, 0],
      ...
      create: { slide: [1, 0], zoom: [0, 1], turn: [-0.125, 0], fade: [0, 1] },
      destroy: { slide: [0, 1], zoom: [1, 0], turn: [0, 0.125], fade: [1, 0] },
    };
    ...
    for (x of afx) {
      const xs = x.split("=");
      if (tfuncs[xs[0]]) {
        if (kind == "move" && ![1, 2, 4, 8].includes(dir)) kind = "hit";
        const tweens = xs[1]
          ? xs[1].split(",").map((p) => parseFloat(p) || 0)
  ...
  function calcTweening() {
    ...
    const tween = EasingFunctions[easing](
      1 - clamp(tweentimer / tweeninterval, 0, 1),
    );
  ...
  function getTweening(tween, drawpos, object, posIndex) {
    ...
    if (dir >= 0) {
      ...
      return [
        drawpos.x - cellwidth * delta[0] * tween,
        drawpos.y - cellheight * delta[1] * tween,
        1,
      ];
    }
  }
    ...
    for (var i = minMaxIJ[0]; i < minMaxIJ[2]; i++) {
      for (var j = minMaxIJ[1]; j < minMaxIJ[3]; j++) {
        ...
        if (
          ...
          ctx.drawImage(
            glyphHighlightDiff,
            xoffset + (i - minMaxIJ[0]) * cellwidth,
            yoffset + (j - minMaxIJ[1]) * cellheight,
          );
        }
      }
    }
    ...
    for (var i = minMaxIJ[0]; i < minMaxIJ[2]; i++) {
      for (var j = minMaxIJ[1]; j < minMaxIJ[3]; j++) {
        ...
        for (var layer = 0; layer < curlevel.layerCount; layer++) {
          var layerMovement = movementbitvec.getshiftor(
            MOV_MASK,
            MOV_BITS * layer,
          );
...
function setClip(tween) {
  ...
  if (debugSwitch.includes("redraw"))
    console.log(
      `clip zfs=${zoomscreen},${flickscreen},${smoothscreen} screen=${screenwidth}x${screenheight} iter=`,
      tween.iter,
    );
  const clipw =
    flickscreen || zoomscreen || smoothscreen
      ? screenwidth
      : tween.iter[2] - tween.iter[0];
  const cliph =
    flickscreen || zoomscreen || smoothscreen
      ? screenheight
      : tween.iter[3] - tween.iter[1];
  const rc = {
    x: xoffset,
    y: yoffset,
    w: clipw * cellwidth,
...
function drawEditorIcons(mini, minj) {
  ...
  // prepare tooltip: content of a level's cell
    ...
    mouseCoordY >= 0 &&
    mouseCoordX < screenwidth &&
    mouseCoordY < screenheight - editorRowCount - 2
  ) {
    const posMask = curLevel.getCellInto(
      mouseCoordY + minj + (mouseCoordX + mini) * curLevel.height,
      _o12,
    );
...
function canvasResize(level) {
  ...
  statusLineHeight = state.metadata.status_line
...
function easingFunction(ease) {
  const e =
    ease in EasingFunctions
      ? ease
      : ease in easingAliases
        ? easingAliases[ease].toLowerCase()
        : "linear";
