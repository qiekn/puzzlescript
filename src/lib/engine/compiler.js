...
var tagDirections = [
  {
    keys: ["^", "v", "<", ">", "perpendicular", "parallel", "orthgonal"],
...
class TagExpander {
  constructor(state, ident, usedirs = false, delim = ":") {
    ...
    if (usedirs) {
      const dirs = this.tail.filter((p) =>
        tagDirections.find((d) => d.keys.includes(p)),
      );
      dirs.forEach((d) => {
        this.keys.push(d);
  ...
  getExpandedAlt(exp, ident) {
    ...
    return tail.length == 0
      ...
      : [
          ...
          ...tail.map(
            (p, x) =>
              this.getPartSubstitute(p, exp) || this.getPartMapped(p, exp) || p,
          ),
        ].join(this.delim);
  }
  ...
  getSubstitutedIdent(exp, ident, index) {
    const newident =
      this.getPartSubstitute(ident, exp) ||
      this.getPartMapped(ident, exp) ||
      ident;
...
function expandObjectDef(state, objid, obj) {
  ...
  return expander.expansion.map((exp, index) => {
    ...
    if (oldobj && !oldobj.canRedef) return [newid, oldobj];
    const newobj = {
      ...deepClone(obj),
      canRedef: true,
    };
    ...
    newobj.cloneSprite =
      obj.cloneSprite && expander.getExpandedAlt(exp, obj.cloneSprite);
    ...
    if (
      newobj.spritematrix.length == 0 &&
      oldobj &&
      oldobj.spritematrix.length > 0
    ) {
...
function applyTransforms(state, obj) {
  ...
  if (obj.vector) setVectorSprite(obj);
  ...
  function setVectorSprite(obj) {
    ...
    if (cloneid) {
      ...
      if (cloneobj && cloneobj.vector) {
        ...
        logError(
          `Canvas object cannot copy from "${errorCase(obj.cloneSprite)}".`,
          obj.lineNumber,
        );
    }
  ...
  function setSpriteMatrix(obj) {
    if (cloneid) {
      ...
      if (cloneobj && !cloneobj.vector) {
        ...
        logError(
          `Sprite object cannot copy from "${errorCase(obj.cloneSprite)}".`,
          obj.lineNumber,
        );
    }
...
function createObjectTagsAsProps(state, ident) {
  const fnValues = (p) =>
    getTag(state, p) || (getMapping(state, p) && getMapping(state, p).values);
  const parts = ident.split(":");
  const todos = parts
    .map((p, x) => ({ part: p, index: x, values: fnValues(p) }))
    .filter((t) => t.values);
  ...
  todos.forEach((todo) => {
    todo.values.forEach((value) => {
      const newident = parts
        .map((p, x) => (x == todo.index ? value : p))
        .join(":");
...
function applySpriteTransforms(obj) {
  const tranfunc = {
    flip: (obj, dir) =>
      [(m) => m.reverse(), (m) => m.map((row) => row.reverse())][dir % 2](
        obj.spritematrix,
      ),
    shift: (obj, dir, amt) =>
      (obj.spritematrix = [
    ...
    rot: (obj, dir1, dir2) =>
      (obj.spritematrix = [
        (m) => m, // 0°
        (m) =>
          Array.from(m[0], (ch, col) => m.map((row) => row[col]).reverse()), // 90°
        (m) => Array.from(m, (l) => l.reverse()).reverse(), // 180°
        (m) =>
          Array.from(m[0], (ch, col) => m.map((row) => row[col])).reverse(), // 270°
  ...
  for (const tf of obj.transforms || []) {
    tranfunc[tf[0]](
      obj,
      cwdIndexOf(tf[1]),
      ["rot", "flip"].includes(tf[0]) ? cwdIndexOf(tf[2]) : +tf[2],
    );
  }
}
...
function applyVectorTransforms(obj) {
  const tranfunc = {
    flip: (obj, dir) =>
      [(p) => (p.flipy = !p.flipy), (p) => (p.flipx = !p.flipx)][dir % 2](
        obj.vector,
      ),
    rot: (prm, dir1, dir2) =>
      [
...
function createObjectRef(state, ident) {
  ...
  if (ref && ref.length > 1) {
    ...
    if (parts.some((e) => relativeDirections.includes(e))) {
      if (debugSwitch.includes("exp"))
        console.log(`Create object ref for relative direction: ${ident}`);
      for (const forward of simpleAbsoluteDirections) {
        const absOf = (dir) =>
          relativeDirs.includes(dir)
            ? relativeDict[forward][relativeDirs.indexOf(dir)]
            : dir;
        const id = ident
          .split(":")
          .map((p) => absOf(p))
...
function isColor(str) {
  ...
  if (/^#([0-9A-F]{3,4})$/i.test(str)) return true;
...
function generateExtraMembers(state) {
  ...
  for (let i = state.collisionLayerGroups.length - 1; i >= 0; --i) {
    ...
    group.firstObjectNo =
      state.objects[state.collisionLayers[group.layer][0]].id;
  ...
  for (const obj of Object.values(state.objects)) {
    if (obj.vector) {
      if (obj.colors.length > 0)
        logWarning(
          `Please don't define colors for vector sprites. They're just going to get ignored anyway.`,
          obj.lineNumber + 1,
        );
    } else if (obj.colors.length == 0) {
      logWarning(
        `A sprite must have at least one color. However did this happen?`,
        obj.lineNumber + 1,
      );
      obj.colors.push("#ff00ff");
    } else if (obj.colors.length > maxColours) {
      logError(
        `A sprite cannot have more than ${maxColours} colors.  Why you would want more than ${maxColours} is beyond me.`,
        obj.lineNumber + 1,
      );
    }
    for (let i = 0; i < obj.colors.length; i++) {
      ...
      if (!isColor(c)) {
        logError(
          `Invalid color specified for object "${n}", namely ${obj.colors[i]}".`,
          obj.lineNumber + 1,
        );
  ...
  if (debugSwitch.includes("obj"))
    console.log("Properties", state.legend_properties);
  if (debugSwitch.includes("obj"))
    console.log("Aggregates", state.legend_aggregates);
  if (debugSwitch.includes("obj"))
    console.log("Synonyms", state.legend_synonyms);
...
function levelAllObjects(state) {
  ...
  const level = new Level(
    ...
    height,
    state.collisionLayers.length,
    new Int32Array(width * height * STRIDE_OBJ),
    "",
  );
...
function levelsToArray(state) {
  ...
  if (state.levels.at(-1).length == 0) state.levels.pop();
  ...
  state.levels.forEach((level) => {
    ...
    if (level[0] == "message") {
      ...
      if (gotoFlag)
        logWarning("GOTO unreachable due to previous GOTO.", level[2]);
      levels.push({
      ...
      logWarning(
        `Option TITLE is not implemented, but may be in the future. Let me know if you really need it.`,
        state.lineNumber,
      );
    } else if (level[0] == "link") {
      links.push({
        // text, will be converted to index later
        target: level[1],
        lineNumber: level[2],
        object: level[3],
      });
  ...
  links.forEach((link) => {
    let index = -9999;
    if (
      (index = levels.findIndex((level) => link.target == level.section)) != -1
    )
      link.targetNo = index;
    else if (
      (index = levels.findIndex((level) => link.target == level.title)) != -1
    )
      link.targetNo = -1 - index;
    else {
      logError(
        `Sorry, link target "${link.target.toUpperCase()}" does not seem to be the name of any level.`,
        link.lineNumber,
      );
      link.targetNo = -9999;
    }
...
function convertSectionNamesToIndices(state) {
  ...
  for (var r = 0; r < state.rules.length; r++) {
    ...
    for (var c = 0; c < rule.commands.length; c++) {
      ...
      if (typeof command[1] == "number") {
        continue;
      } //Variant rule was previously converted into an index PS+#105
      var sectionName = command[1].toLowerCase();
      var sectionIndex = sectionMap[sectionName];
      if (sectionIndex === undefined) {
        logError(
          `Invalid GOTO command - there is no section named "${errorCase(command[1])}". Either it does not exist, or it has zero levels.`,
          rule.lineNumber,
        );
        ...
        logError(
          `Invalid GOTO command - there are multiple sections named "${errorCase(command[1])}". Section names must be unique for GOTO to work.`,
          rule.lineNumber,
        );
        sectionIndex = -9999;
      }
  ...
  for (var i = 0; i < state.levels.length; i++) {
    ...
    if (targetIndex === undefined) {
      logError(
        `Invalid GOTO command - there is no section named "${errorCase(targetName)}".`,
        level.lineNumber,
      );
      targetIndex = 0;
    } else if (duplicateSections[targetName] !== undefined) {
      logError(
        `Invalid GOTO command - there are multiple sections named "${errorCase(targetName)}".`,
        level.lineNumber,
      );
      targetIndex = 0;
    }
  ...
  for (const group of rules) {
    for (const rule of group) {
      for (const cmd of rule.commands) {
        ...
        if (cmd[0] == "gosub" && typeof cmd[1] == "string") {
          // the vagaries of the parse means this fixup may already have been done
          const subroutine = subroutines.find(
            (s) => s.label == cmd[1].toLowerCase(),
          );
          if (!subroutine) {
            logError(
              `Invalid GOSUB command - there is no subroutine named "${cmd[1]}".`,
              rule.lineNumber,
            );
          } else {
            cmd[1] = rules.findIndex(
              (r) => r[0].lineNumber >= subroutine.lineNumber,
            );
            if (cmd[1] == -1)
              logWarning(
                `GOSUB to an empty subroutine -- or a late/non-late mixup?`,
                rule.lineNumber,
              );
          }
        }
  ...
  for (const row of [...rule.lhs, ...rule.rhs]) {
    ...
    for (const cell of row) {
      for (var k = 0; k < cell.length; k += 2) {
        if (relativeDirections.includes(cell[k])) return true;
...
*/
      ...
      case 0: {
        ...
        if (token === "+") {
          ...
          if (groupNumber === lineNumber) {
            ...
            } else groupNumber = curRules[curRules.length - 1].groupNumber;
          } else {
            logError(
              'Two "+"s (the "append to previous rule group" symbol) applied to the same rule.',
              lineNumber,
            );
          }
      ...
      case 1: {
        if (token == "[") {
          ...
          if (!incellrow) {
            ...
            logError(
              "In a rule, if you specify a movement, it has to act on an object.",
              lineNumber,
            );
          } else {
          ...
          if (groupNumber !== lineNumber) {
            ...
            if (parentrule.late !== late) {
              logWarning(
                "Oh gosh you can mix late and non-late rules in a rule-group if you really want to, but gosh why would you want to do that?  What do you expect to accomplish?",
                lineNumber,
              );
            }
          }
          ...
          wordAlreadyDeclared(state, token) ||
          createObjectRef(state, token)
        ) {
          ...
          if (needarg || twid) {
            if (twid && !state.metadata.runtime_metadata_twiddling) {
              ...
              if (twid && str == "")
                logError(
                  'You included a twiddleable option, but did not specify a value. The twiddle may behave strangely. Please use "set", "default", "wipe", or specify the correct value. See the documentation for more info.',
                  lineNumber,
                );
  ...
  if (late && rigid) {
    logError(
      "Late rules cannot be marked as rigid (rigid rules are all about dealing with the consequences of unresolvable movements, and late rules can't even have movements).",
      lineNumber,
    );
  }
...
function rulesToArray(state) {
  ...
  for (const rule of rules) convertRelativeDirsToAbsolute(state, rule);
...
function parseRulesToArray(state) {
  ...
  for (var i = 0; i < oldrules.length; i++) {
    ...
    } else if (newrule.label) {
      // PS>
      const other = subroutines.find((s) => s.label == newrule.label);
      if (other)
        logError(
          `Duplicate subroutine, "${newrule.label}" already defined at line ${other.lineNumber}`,
          newrule.lineNumber,
        );
      else {
...
function expandRulesWithPrefixes(state, rules) {
  ...
  if (debugSwitch.includes("exp"))
    console.log(
      `expandRulesWithPrefixes ${rules.length} -> ${newrules.length}`,
    );
  return newrules;
}
...
function expandRuleWithPrefixes(state, rule) {
  ...
  const cartesian = cartesianProduct(
    ...rule.prefixes.map((p) => state.tags[p]),
  );
    ...
    row.forEach((cell, cellx) => {
      ...
      cell.forEach((atom, atomx) => {
        if (
          ...
          cell[atomx - 1] != "no" &&
          atom != "..." &&
          !wordAlreadyDeclared(state, atom)
        ) {
  ...
  todo.forEach((t, tx) => {
    ...
    newrules.forEach((r) => {
      ...
      for (const newident of expander.getExpandedIdents()) {
        ...
        if (
          rule.rhs.length > 0 &&
          rule.rhs[t.rowx][t.cellx][t.atomx] == t.ident
        )
...
function expandRulesWithMultiDirectionObjects(state, rules) {
  ...
  for (const rule of rules) {
    ...
    const dirs = objs
      ...
      .flat()
      .filter(
        (part, x, a) => part in directionaggregates && a.indexOf(part) == x,
      );
    if (dirs.length == 0) newrules.push(rule);
    else {
      const muldir = dirs[0];
  ...
  if (debugSwitch.includes("exp"))
    console.log(
      `expandRulesWithMultiDirectionObjects ${rules.length} -> ${newrules.length}`,
    );
  return newrules;
}
...
function expandRulesWithMultipleDirections(state, rules) {
  ...
  if (debugSwitch.includes("exp"))
    console.log(
      `expandRulesWithMultipleDirections ${rules.length} -> ${newrules.length}`,
    );
  return newrules;
}
...
function convertObjectsAndDirections(state, rules2) {
  ...
  for (var i = 0; i < rules3.length; i++) {
    var rule = rules3[i];
    rules4 = rules4.concat(
      concretizePropertyRule(state, rule, rule.lineNumber),
    );
  }
...
function rewriteUpLeftRules(rule) {
  ...
  if (rule.rhs.length > 0 && rule.rhs.length != rule.lhs.length)
    // safe: can happen with prior error
    return;
...
function expandRuleTags(state, cell) {
  ...
  for (var i = 0; i < cell.length; i += 2) {
    ...
    if (
      name.includes(":") &&
      (dir == "no" || !(name in state.propertiesDict))
    ) {
...
function concretizePropertyRule(state, rule, lineNumber) {
  ...
  for (var i = 0; i < rule.lhs.length; i++) {
    ...
    for (var j = 0; j < cur_cellrow_l.length; j++) {
      cur_cellrow_l[j] = expandNoPrefixedProperties(
        state,
        expandRuleTags(state, cur_cellrow_l[j]),
      );
      if (rule.rhs.length > 0)
        rule.rhs[i][j] = expandNoPrefixedProperties(
          state,
  ...
  if (mappingProperties_l.length > 0 && mappingProperties_r.length > 0) {
    ...
    if (
      !mappingProperties_l.some((value) => proplen(value) != len0) &&
  ...
  function replaceMappedProperties(oldRule, numprops) {
    let newRules = [];
    while (numprops-- > 0) newRules.push(deepCloneRule(oldRule));
    ...
    if (debugSwitch.includes("exp"))
      console.log(
        `replaceMappedProperties added ${rules.length} -> ${newrules.length}`,
      );
    return newRules;
  }
        ...
        for (var l = 0; l < cell_r.length; l += 2) {
          ...
          if (object_dir === "...") {
            ...
            if (object_name in state.objectMasks) {
              ...
              if (state.propertiesDict.hasOwnProperty(object_name)) {
                ...
                logWarning(
                  `In this rule you're asking me to spawn a random ${object_name.toUpperCase()} for you, but that's already a concrete single object.  You wanna be using random with properties (things defined in terms of OR in the legend) so there's some things to select between.`,
                  rule.lineNumber,
                );
                values = [object_name];
              }
          ...
          if (object_dir == "no") {
            ...
            if (object_dir.length > 0) {
              postMovementsLayerMask_r.ishiftor(
                MOV_MASK,
                MOV_BITS * layerIndex,
              );
            }
            ...
            if (object_dir === "randomdir") {
              randomDirMask_r.ishiftor(
                dirMasks[object_dir],
                MOV_BITS * layerIndex,
              );
            } else {
              movementsSet.ishiftor(
                dirMasks[object_dir],
                MOV_BITS * layerIndex,
              );
            }
      ...
      for (var j = 0; j < oldrule.lhs.length; j++) {
        ...
        for (var k = 0; k < cellrow_l.length; k++) {
          if (cellrow_l[k] === ellipsisPattern) {
            ...
            if (ellipses[j] > 2) {
              logError(
                "You can't use more than two ellipses in a single cell match pattern.",
                oldrule.lineNumber,
              );
            } else {
              if (k > 0 && cellrow_l[k - 1] === ellipsisPattern) {
                logWarning(
                  "Why would you go and have two ellipses in a row like that? It's exactly the same as just having a single ellipsis, right?",
                  oldrule.lineNumber,
                );
              }
            }
...
function ruleGroupDiscardOverlappingTest(ruleGroup) {
  ...
  for (var i = 0; i < ruleGroup.length; i++) {
    ...
    if (rule.hasOwnProperty("discard")) {
      var beforesame =
        i === 0 ? false : ruleGroup[i - 1].lineNumber === rule.lineNumber;
      var aftersame =
      ...
      if (!(beforesame || aftersame) || ruleGroup.length === 0) {
        ...
        logError(
          `${example[0]} and ${example[1]} can never overlap${parenthetical}, but this rule requires that to happen, so it's being culled.`,
          rule.lineNumber,
        );
      }
      i--;
...
function generateRigidGroupList(state) {
  ...
  if (rigidGroupIndex_to_GroupIndex.length > 30) {
    ...
    logError(
      "There can't be more than 30 rigid groups (rule groups containing rigid members).",
      state.rules[group_index][0].lineNumber,
    );
  }
...
function twiddleMetaData(state, command = null) {
  if (debugSwitch.includes("meta"))
    console.log(`twiddleMetaData update=${command} metadata:`, state.metadata);
  var newmetadata;
  ...
  function getIntCheckedPositive(s, lineNumber) {
    if (!isFinite(s) || !isInt(s)) {
      logWarning(
        `Wasn't able to make sense of "${s}" as a (whole number) dimension.`,
        lineNumber,
      );
      return NaN;
    }
    ...
    if (isNaN(result)) {
      logWarning(
        `Wasn't able to make sense of "${s}" as a dimension.`,
        lineNumber,
      );
    }
    if (result <= 0) {
      logWarning(
        `The dimension given to me (you gave "${s}") is baad - it should be greater than 0.`,
        lineNumber,
      );
    }
    return result;
  ...
  function getCoords(str, lineNumber) {
    ...
    if (coords.length !== 2) {
      ...
      var intcoords = [
        getIntCheckedPositive(coords[0], lineNumber),
        getIntCheckedPositive(coords[1], lineNumber),
      ];
      if (
        !isFinite(coords[0]) ||
        !isFinite(coords[1]) ||
        ...
        logWarning(
          `Couldn't understand the dimensions given to me (you gave "${str}") - should be of the form AxB.`,
          lineNumber,
        );
        return null;
      } else {
        if (intcoords[0] <= 0 || intcoords[1] <= 0) {
          logWarning(
            `The dimensions given to me (you gave "${str}") are baad - they should be > 0.`,
            lineNumber,
          );
        }
        return intcoords;
  ...
  function parseTwiddle(newmetadata) {
    if (newmetadata.flickscreen !== undefined) {
      ...
      newmetadata.flickscreen = getCoords(
        val,
        state.metadata_lines.flickscreen,
      );
    ...
    if (newmetadata.smoothscreen !== undefined) {
      ...
      if (args.length < 1) {
        ...
        logErrorNoLine(
          `Smoothscreen given "${args.length}" arguments but expects at most 4: smoothscreen [flick] WxH [IxJ] [S].`,
        );
        validArguments = false;
      }
      ...
      if (screenSizeMatch) {
        ...
        smoothscreen.screenSize.height = parseInt(
          screenSizeMatch.groups.height,
        );
        ...
        logErrorNoLine(
          `Smoothscreen given first argument "${args[0]}" but must be formatted WxH where W and H are integers.`,
        );
        validArguments = false;
      }

      if (args.length > 1) {
        const boundarySizeMatch = args[1].match(
          /^(?<width>\d+)x(?<height>\d+)$/,
        );
        if (boundarySizeMatch) {
          ...
          logErrorNoLine(
            `Smoothscreen given second argument "${args[1]}" but must be formatted IxJ where I and J are integers.`,
          );
          validArguments = false;
        }
      }
      ...
      if (args.length > 2) {
        ...
        if (cameraSpeedMatch) {
          ...
          logErrorNoLine(
            `Smoothscreen given third argument "${args[2]}" but must be a number.`,
          );
          validArguments = false;
        }
      }
    ...
    if (newmetadata.tween_easing) {
      ...
      if (easing) {
        easing =
          !isNaN(parseInt(easing)) && easingAliases[parseInt(easing)]
            ? easingAliases[parseInt(easing)]
            : easing.toLowerCase();
        if (EasingFunctions[easing]) newmetadata.tween_easing = easing;
        else {
          logErrorNoLine(
            `Sorry, but tween easing argument "${newmetadata.tween_easing}" is not a valid number or lerp.`,
          );
    ...
    if (newmetadata.tween_snap) {
      ...
      if (snap) newmetadata.tween_snap = snap;
        ...
        logErrorNoLine(
          `Sorry, but tween snap argument "${newmetadata.tween_snap}" is not a valid number.`,
        );
    ...
    if (newmetadata.color_palette) {
      ...
      const palette =
        args[0] in colorPalettesAliases
          ? colorPalettesAliases[args[0]]
          : args[0];
      if (!(palette in colorPalettes)) {
        logError(
          `Palette "${palette}" not found, defaulting to arnecolors.`,
          0,
        );
        newmetadata.color_palette = colorPalettes.arnecolors;
      } else {
  ...
  for (var i = 0; i < state.winconditions.length; i++) {
    ...
    if (n1 in state.objectMasks) {
      ...
      logError(
        'Unwelcome term "' +
          n1 +
          "\" found in win condition. I don't know what I'm supposed to do with this. ",
        lineNumber,
      );
    }
...
function cacheRuleStringRep(rule) {
  ...
  for (var i = 0; i < rule.commands.length; i++) {
    ...
    result +=
      command.length == 1 ? ` ${command[0]}` : ` ${command[0]}(${command[1]})`;
  }
  //print commands next
...
function printRules(state) {
  ...
  for (var i = 0; i < state.rules.length; i++) {
    ...
    let subrtext = "";
    for (
      let subroutine = state.subroutines[subroutineIndex];
    ...
    if (rule.hasOwnProperty("discard")) {
      ...
      var sameGroupAsPrevious =
        i > 0 && state.rules[i - 1].groupNumber === rule.groupNumber;
  ...
  if (loopIndex < state.loops.length) output += "ENDLOOP<br>";
  output += "=============<br>";
  output =
    "<br>Rule Assembly : (" +
  ...
  if (state.loops.length > 0) {
    for (var i = 0; i < state.loops.length; i++) {
      ...
      if (i % 2 === 0) {
        ...
        if (loop[1] === 1) {
          logError(
            "Found a STARTLOOP, but I'm already inside a loop? (Puzzlescript can't nest loops, FWIW).",
            loop[0],
          );
        }
      }
    ...
    if (lastloop[1] !== -1) {
      logError(
        "Yo I found a STARTLOOP without a corresponding ENDLOOP.",
        lastloop[0],
      );
    }
  ...
  for (var j = 0; j < state.loops.length; j++) {
    ...
    for (var i = 0; i < state.rules.length; i++) {
      ...
      if (loop[0] >= firstRuleLine && loop[0] <= lastRuleLine) {
        logWarning(
          "Found a loop point in the middle of a rule. You probably don't want to do this, right?",
          loop[0],
        );
      }
      if (outside) {
        ...
        if (firstRuleLine >= loop[0]) {
          ...
          if (source >= target) loopPoint[source] = target;
          outside = true;
          break;
        }
      }
  ...
  for (const sound of state.sounds) {
    if (sound[0] === "SOUNDEVENT") {
      ...
      if (soundverbs_directional.includes(verb)) {
        if (directions.length == 0) directions = ["orthogonal"];
      } else if (soundverbs_movement.includes(verb)) {
        directions = [`_${verb}_`];
        verb = "move";
      } else if (directions.length > 0)
        logError(
          "Incorrect sound declaration - cannot have directions (UP/DOWN/etc.) attached to non-directional sound verbs (CREATE is not directional, but MOVE is directional).",
          line,
        );
      ...
      for (const dir of directions) {
        ...
        else directionMask |= soundDirectionMasks[dir];
      }
      ...
      for (let j = 0; j < targets.length; j++) {
        ...
        if (debugSwitch.includes("sfx"))
          console.log(`Sfx verb ${verb} o: ${JSON.stringify(o)}`);

        if (verb === "move") sfx_MovementMasks[targetLayer].push(o);
        else if (verb === "cantmove") sfx_MovementFailureMasks.push(o);
        else if (verb === "create")
          sfx_CreationMasks.push(o); // (verb === 'destroy')
        else sfx_DestructionMasks.push(o);
      }
    }
  }
...
function formatHomePage(state) {
  ...
  state.bgcolor =
    "background_color" in state.metadata
      ? colorToHex(
          state.metadata.color_palette,
          state.metadata.background_color,
        )
      : "#000000";
  state.fgcolor =
    "text_color" in state.metadata
      ? colorToHex(state.metadata.color_palette, state.metadata.text_color)
      : "#FFFFFF";
  state.author_color =
    "author_color" in state.metadata
      ? colorToHex(state.metadata.color_palette, state.metadata.author_color)
      : "#FFFFFF";
  state.title_color =
    "title_color" in state.metadata
      ? colorToHex(state.metadata.color_palette, state.metadata.title_color)
      : "#FFFFFF";
  state.keyhint_color =
    "keyhint_color" in state.metadata
      ? colorToHex(state.metadata.color_palette, state.metadata.keyhint_color)
      : "#FFFFFF"; // todo:
...
function loadFile(str) {
  ...
  if (debugSwitch.includes("layer"))
    console.log("Collision Layers", state.collisionLayers);
  if (debugSwitch.includes("layer"))
    console.log("Collision Layer Groups", state.collisionLayerGroups);
  ...
  if (debugSwitch.includes("rules"))
    console.log("Subroutines", state.subroutines);
  ...
  if (debugSwitch.includes("rules"))
    console.log("Loop Points", state.loopPoint);
  if (debugSwitch.includes("rules"))
    console.log("Late Loop Points", state.lateLoopPoint);
...
function compile(command, text, randomseed) {
  ...
  if (!state || state.invalid) {
    console.log(
      `End compile: aborted with error ${errorStrings[errorStrings.length - 1]}`,
    );
    consoleError(
      `<span class="systemMessage">Compilation aborted. ${errorStrings[errorStrings.length - 1]}</span>`,
    );
    return null;
  }

  console.log(
    `End compile: ${state && !state.invalid && errorCount == 0 ? "ok" : "FAILED, error count = " + errorCount}`,
  );
  ...
  if (errorCount > 0) {
    ...
    consolePrint(
      htmlClass(
        "systemMessage",
        `Successful ${command[0] == "restart" ? "compilation" : "live recompilation"}, generated ${ruleCount} instructions.`,
      ),
    );
    ...
    if (debugMode) {
      consolePrint(
        htmlClass(
          "systemMessage",
          `Tags: ${Object.keys(state.tags).length} Objects: ${state.objectCount} Layers: ${state.collisionLayers.length} Sounds: ${state.sounds.length} Levels: ${state.levels.length}.`,
        ),
      );
    }
  ...
  if (IDE) {
    if (
      ...
      logWarning(
        "Using tweens in a game that also has LATE rules is currently experimental! If you change objects that moved with LATE then tweens might not play!",
        undefined,
        true,
      );
      logWarning(
        "Note that if you change objects that have moved in LATE rules, their tweens won't play!",
        undefined,
        true,
      );
    }
    ...
    if (
      ...
      logWarning(
        "You can't use both level_select_unlocked_ahead and level_select_unlocked_rollover at the same time, so please choose only one!",
        undefined,
        true,
      );
    }
    ...
    if (
      ...
      logWarning(
        "You're using level select prelude flags, but didn't define the 'level_select' flag.",
        undefined,
        true,
      );
    }
    ...
    if (
      ...
      logWarning(
        "You've defined a level unlock condition, but didn't define the 'level_select_lock' flag.",
        undefined,
        true,
      );
    }
    ...
    if (state.metadata.runtime_metadata_twiddling_debug !== undefined) {
      logWarning(
        "RUNTIME_METADATA_TWIDDLING_DEBUG is deprecated, however you can now use verbose logging instead to see when metadata is changed.",
        undefined,
        true,
      );
    }
    ...
    if (
      ...
      logWarning(
        "To use LEVEL_SELECT, you need to create some sections first, otherwise the level list will be empty! Please see the docs for more info.",
        undefined,
        true,
      );
    }
