...
function getTilesTraversingPoints(x1, y1, x2, y2) {
  ...
  var scaledAbsDeviationTimesTwo =
    cellwidth * (Math.abs(dirX) + Math.abs(dirY)); // == abs(horizontalDeviation*dirY*2)
  ...
  while (cellX1 !== cellX2 || cellY1 !== cellY2) {
    if (
      cellY1 >= curLevel.height ||
      cellY1 < 0 ||
      cellX1 >= curLevel.width ||
      cellX1 < 0
    ) {
      error =
    ...
    hitObstacle = function () {
      var coordIndex =
        screenOffsetY + cellY1 + (screenOffsetX + cellX1) * curLevel.height;
...
function mouseAction(event, click, id) {
  if (debugSwitch.includes("input"))
    console.log("mouseAction", event, click, id);
  if (textMode) {
    ...
    if (titleScreen) {
      ...
      if (titleMode == 0) {
        ...
        if (titleSelection) titleButtonSelected();
      } else if (titleMode === 2) {
        //Level select
      ...
      messageselected === false &&
      (state.levels[curLevelNo].message || messagetext != "")
    ) {
    ...
    if (!click) {
      ...
      for (var i = 0; i < tileListX.length; i++) {
        var coordIndex =
          screenOffsetY +
          tileListY[i] +
          (screenOffsetX + tileListX[i]) * curLevel.height;
      ...
      var coordIndex =
        screenOffsetY +
        mouseCoordY +
        (screenOffsetX + mouseCoordX) * curLevel.height;
      if (againing) {
        //consolePrint("no mouse, againing",false);
      } else {
        pushInput(`mouse,${id},${coordIndex}`);
...
function checkKey(e, justPressed) {
  if (debugSwitch.includes("input") && justPressed)
    console.log("checkKey", prevTimestamp, e, justPressed);
  ...
  if (e.keyCode >= 0x70 && e.keyCode <= 0x83)
    // function keys
    return;
