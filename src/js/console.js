...
document
  .getElementById("clearConsoleClick")
  .addEventListener("mousedown", clearConsole, false);

function verboseToggle(event) {
  defaultVerboseLogging = !defaultVerboseLogging;
  consolePrint(
    `Default verbose logging is now ${defaultVerboseLogging ? "ENABLED" : "DISABLED"}`,
    true,
  );
...
document
  .getElementById("verboseLoggingClick")
  .addEventListener("mousedown", verboseToggle, false);

function debugToggle() {
  defaultDebugMode = !defaultDebugMode;
  runClick();
  consolePrint(
    `Default compiler debug mode is now ${defaultDebugMode ? "ENABLED" : "DISABLED"}`,
    true,
  );
}
...
function showLayersToggle() {
  ...
  consolePrint(
    `Show layers is now ${showLayers ? "ON. Use PgUp/PgDn to change layers." : "OFF."}`,
    true,
  );
  canvasResize();
