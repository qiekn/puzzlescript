...
function makeGIF() {
  ...
  consolePrint(
    `<img class="generatedgif" src="${data_url}" width="${gifcanvas.width}px" height="${gifcanvas.height}px">`,
    true,
  );
  const gametitle = state.metadata.title
  ...
  consolePrint(
    '<a href="' + data_url + '" download="' + filename + '">Download GIF</a>',
    true,
  );
