...
function generateTitleScreen() {
  ...
  if (state.metadata.author !== undefined) {
    author_lines = wordwrap(
      expandCJKCharacters("by " + state.metadata.author),
      TERMINAL_WIDTH,
    );
  ...
  if (titleMode === 0) {
    ...
    if (titleSelection === 0) {
      layout_rows.menu_options.push(
        titletemplate_menu_newgame[titleSelected ? 2 : 1],
      );
      layout_rows.menu_options.push(blank_row);
      ...
      layout_rows.menu_options.push(blank_row);
      layout_rows.menu_options.push(
        titletemplate_menu_continue[titleSelected ? 2 : 1],
      );
    }
  }
  ...
  if (bs + t_len + bm + a_len + be > header_size) {
    ...
    if (title_trimmed) {
      logWarning(
        "Game title is too long to fit on screen; truncating to fit.",
    ...
    if (author_trimmed) {
      logWarning(
        "Author name is too long to fit on screen; truncating to fit.",
  ...
  if (titleSelected) {
    ...
    if (!loading_text) {
      ...
      titleImage[selection_row] =
        spaces +
        "#" +
        titleImage[selection_row].slice(
          left_index_to_replace + 1,
          right_index_to_replace,
        ) +
        "#" +
        spaces;
    } else {
...
function generate_moveEntitiesAtIndex(OBJECT_SIZE, MOVEMENT_SIZE) {
  ...
  return (CACHE_MOVEENTITIESATINDEX[fn] = new Function(
    "level",
    "positionIndex",
    "entityMask",
    "dirMask",
...
function generate_repositionEntitiesAtCell(OBJECT_SIZE, MOVEMENT_SIZE) {
  ...
  return (CACHE_REPOSITIONENTITIESATCELL[fn] = new Function(
    "level",
    "positionIndex",
    fn,
  ));
...
function Rule(rule) {
  ...
  this.applyAt = this.generateApplyAt(
    this.patterns,
    this.ellipsisCount,
    STRIDE_OBJ,
    STRIDE_MOV,
  ...
  if (ellipsisCount === 0) {
    ...
    for (let i = 0; i < cr_l; i++) {
      ...
      for (let j = 0; j < STRIDE_MOV; j++) {
        if (
          pattern.movementsPresent.data[j] ||
          pattern.movementsMissing.data[j]
        ) {
    ...
    return (CACHE_RULE_CELLROWMATCHESFUNCTION[fn] = new Function(
      "cellRow",
      "i",
      "d",
      "objects",
...
CellPattern.prototype.replace = function (level, rule, currentIndex) {
  let fn = this.generateReplaceFunction(STRIDE_OBJ, STRIDE_MOV, rule);
  this.replace = fn;
...
CellPattern.prototype.generateReplaceFunction = function (
  OBJECT_SIZE,
  MOVEMENT_SIZE,
  rule,
) {
  ...
  for (let i = 0; i < OBJECT_SIZE; i++) {
    ...
    key_array[i + 2 * OBJECT_SIZE + 3 * MOVEMENT_SIZE] =
      this.replacement.randomEntityMask.data[i] || 0;
  }
  for (let i = 0; i < MOVEMENT_SIZE; i++) {
    key_array[i + 2 * OBJECT_SIZE] = this.replacement.movementsSet.data[i] || 0;
    key_array[i + 2 * OBJECT_SIZE + MOVEMENT_SIZE] =
      this.replacement.movementsClear.data[i] || 0;
    key_array[i + 2 * OBJECT_SIZE + 2 * MOVEMENT_SIZE] =
      this.replacement.movementsLayerMask.data[i] || 0;
    key_array[i + 3 * OBJECT_SIZE + 3 * MOVEMENT_SIZE] =
      this.replacement.randomDirMask.data[i] || 0;
  }
  ...
  const replace_randomEntityMask_zero =
    this.replacement.randomEntityMask.iszero();
  const replace_randomDirMask_zero = this.replacement.randomDirMask.iszero();
  let deterministic =
    replace_randomEntityMask_zero && replace_randomDirMask_zero;
  ...
  return (CACHE_CELLPATTERN_REPLACEFUNCTION[key] = new Function(
    "level",
    "rule",
    "currentIndex",
    fn,
...
function generateMatchCellRow(OBJECT_SIZE, MOVEMENT_SIZE) {
  ...
  return (CACHE_MATCHCELLROW[fn] = new Function(
    "level",
    "direction",
    "cellRowMatch",
    "cellRow",
...
function generateMatchCellRowWildCard(OBJECT_SIZE, MOVEMENT_SIZE) {
  ...
  return (CACHE_MATCHCELLROWWILDCARD[fn] = new Function(
    "direction",
    "cellRowMatch",
    "cellRow",
    "cellRowMask",
...
Rule.prototype.findMatches = function () {
  ...
  for (
    ...
    if (this.ellipsisCount[cellRowIndex] === 0) {
      ...
      // ellipsiscount===1/2
      match = state.matchCellRowWildCard(
        this.direction,
        matchFunction,
        cellRow,
        cellRowMasks[cellRowIndex],
...
Rule.prototype.generateApplyAt = function (
  patterns,
  ellipsisCount,
  OBJECT_SIZE,
  MOVEMENT_SIZE,
  ...
  return (CACHE_RULE_APPLYAT[fn] = new Function(
    "level",
    "tuple",
    "check",
    "delta",
...
function applyRules(rules, loopPoint, bannedGroup) {
  ...
  while (ruleGroupIndex < RULES_COUNT) {
    ...
    if (
      ...
      loopPropagated &&
      loopPoint[ruleGroupIndex] !== undefined
    ) {
      ruleGroupIndex = loopPoint[ruleGroupIndex];
      loopPropagated = false;
      loopCount++;
...
function generate_resolveMovements(OBJECT_SIZE, MOVEMENT_SIZE, state) {
  ...
  return (CACHE_RESOLVEMOVEMENTS[fn] = new Function(
    "level",
    "bannedGroup",
    fn,
  ));
...
function processInput(dir, dontDoWin, dontModify) {
  ...
  if (verbose_logging) {
    ...
    if (dir === -1) {
      ...
      consolePrint(
        `Turn starts with input of ${["up", "left", "down", "right", "action"][inputindex]}.`,
        false,
        null,
        inspect_ID,
      );
  ...
  do {
    ...
    if (shouldUndo) {
      ...
      if (IDE) {
        ...
        let bannedLineNumbers = newBannedGroups.map(
...
function processCommandQueue(bak, dontModify, dontDoWin, inputDir) {
  ...
  if (
    dontModify &&
    (level.commandQueue.includes("win") ||
      level.commandQueue.includes("restart"))
  ) {
    return true;
  }
...
Rule.prototype.generateFindMatchesFunction = function () {
  ...
  for (let i = 0; i < this.patterns.length; i++) {
    ...
    if (this.ellipsisCount[i] === 0) {
      fn +=
        `match${i} = state.matchCellRow(level,this.direction, this.cellRowMatches[${i}], ` +
        `this.patterns[${i}], this.cellRowMasks[${i}], ` +
        `this.cellRowMasks_Movements[${i}], d);\n`;
    } else if (this.ellipsisCount[i] === 1) {
      fn +=
        `match${i} = state.matchCellRowWildCard(this.direction, this.cellRowMatches[${i}], ` +
        `this.patterns[${i}], this.cellRowMasks[${i}], ` +
