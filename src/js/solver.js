...
function distance(index1, index2) {
  return (
    Math.abs(
      Math.floor(index1 / curLevel.height) -
        Math.floor(index2 / curLevel.height),
    ) + Math.abs((index1 % curLevel.height) - (index2 % curLevel.height))
  );
}
      ...
      if (wincondition[0] == -1) {
        ...
        for (var i = 0; i < curLevel.n_tiles; i++) {
          if (!filter1.bitsClearInArray(curLevel.getCellInto(i, _o10).data)) {
            ...
            for (var j = 0; j < curLevel.n_tiles; j++) {
              if (
                !filter2.bitsClearInArray(curLevel.getCellInto(j, _o10).data)
              ) {
