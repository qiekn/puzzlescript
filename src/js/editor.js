...
function tryLoadFile(fileName, docompile, doclear) {
  ...
  fileOpenClient.onreadystatechange = function () {
    if (fileOpenClient.readyState != 4) return;
    if (fileOpenClient.status != 200 && fileOpenClient.status != 201) {
      consoleError(
        "HTTP Error " +
          fileOpenClient.status +
          " - " +
          fileOpenClient.statusText,
      );
    } else loadGame(fileOpenClient.responseText, docompile, doclear);
  };
  fileOpenClient.send();
}
...
function addToDebugTimeline(level, lineNumber) {
  ...
  var debugTimelineSnapshot = {
    ...
    rigidMovementAppliedMask: norigidlog
      ? level.rigidMovementAppliedMask
      : level.rigidMovementAppliedMask.map((a) => a.clone()),
