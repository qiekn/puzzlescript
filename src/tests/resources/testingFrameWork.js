function runTest(dataarray, testname) {
  ...
  if (levelString !== dataarray[2]) {
    ...
    QUnit.assert.equal(
      levelString,
      dataarray[2],
      "Resulting level state is not the expected one.",
    );
  }
  ...
  if (audio_output !== null) {
    ...
    if (audio_recorded != audio_expected) {
      ...
      QUnit.assert.equal(
        audio_recorded,
        audio_expected,
        "Audio output is not as expected",
      );
    }
  }
  if (success) {
...
function runCompilationTest(dataarray, testname) {
  ...
  for (
    i_simulated = 0;
    i_simulated < strippedErrorStrings.length &&
    i_recorded < recordedErrorStrings.length;
    i_simulated++
  ) {
