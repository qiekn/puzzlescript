  ...
  QUnit.test(
    ...
    (function (tdat) {
      return function () {
        ...
        QUnit.assert.true(
          runTest(tdat, checkerrors),
          "Passed all tests" +
            tdDescription +
            (errstrings.length > 0
              ? listify("Actual errors", errstrings)
              : "") +
            (soundHistory.length > 0
              ? lineof("Actual sounds", soundHistory.join())
              : "") +
            (QUnit.config.showsource
              ? lineof("Game source", `<pre>${tdCode}</pre>`)
              : ""),
        );
      };
    })(testData),
...
QUnit.testDone((details) => {
  if (testLookup[details.testId]) {
    ...
    if (ele) {
      ...
      eleA.insertAdjacentHTML(
        "afterend",
        `<a id="openClickLink-${details.testId}" href="javascript:void('Open in editor');">Open</a>`,
      );
      document
        .getElementById(`openClickLink-${details.testId}`)
...
function addStartLink() {
  ...
  ele
    ...
    .insertAdjacentHTML(
      "afterend",
      `<a id="startLink" href="javascript:void('Start the run');">Start</a>`,
    );
...
function listify(label, s) {
  return (
    `<b>${label}:</b><ul>` +
    s.map((m) => "<li>" + JSON.stringify(stripHTMLTags(m)) + "</li>").join("") +
    "</ul>"
  );
}
